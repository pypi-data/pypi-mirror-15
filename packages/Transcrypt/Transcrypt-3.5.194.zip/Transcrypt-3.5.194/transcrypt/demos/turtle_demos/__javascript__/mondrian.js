"use strict";
// Transcrypt'ed from Python, 2016-06-27 16:34:20
function mondrian () {
	var __all__ = {};
		...
		var originalColor = function () {
			while (true) {
				...
				if (result == 'white' || !(__in__ (result, recentColors))) {
					...
					print (result, __kwargdict__ ({end: ' '}));
					return result;
				}
			}
		};
		var rect = function (xMin, yMin, xMax, yMax) {
			var __iterable0__ = tuple (['black', originalColor ()]);
			for (var __index0__ = 0; __index0__ < __iterable0__.length; __index0__++) {
				var aColor = __iterable0__ [__index0__];
				color (aColor, aColor);
				up ();
				goto (xMin, yMin);
				down ();
