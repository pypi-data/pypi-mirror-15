...
class Image2PyArray:
    ...
    def from_dir(self, dirname):
        ...
        train_XY = get_XY_tuple(os.path.join(dirname, "train")) 
        test_XY = get_XY_tuple(os.path.join(dirname, "test")) 
    ...
    def get_XY_tuple(self, dirname):
        ...
        labels = getdirs(dirname)
        for label in labels:
            labeldir = os.path.join(dirname, label)
            for filename in getfiles(labeldir):
                filepath = os.path.join(labeldir, filename)
                X.append(pic2array(filepath))
                Y.append(label2array(label,labels))
        return (X, Y)

    def pic2array(self, filepath):
        return numpy.asarray(Image.open(filepath).convert('L'))
    ...
    def check_dir_tree(self, dirname):
        ...
        if set(getdirs(train)) != set(getdirs(test)):
