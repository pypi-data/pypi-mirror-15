...
def drop_database(db_name, conn_dict):
    execute_sql(
        "DROP DATABASE IF EXISTS {0};".format(db_name), conn_dict
    )


def create_database(db_name, conn_dict):
    execute_sql(
...
def drop_and_create(target_db_conn_dict):
    target_db_name = target_db_conn_dict['db']
    target_connection = {
        key: value for (key, value) in target_db_conn_dict.iteritems()
        if key != 'db'
    }
    print "Dropping and creating the database {0}.".format(target_db_name)
    drop_database(target_db_name, target_connection)
    create_database(target_db_name, target_connection)
