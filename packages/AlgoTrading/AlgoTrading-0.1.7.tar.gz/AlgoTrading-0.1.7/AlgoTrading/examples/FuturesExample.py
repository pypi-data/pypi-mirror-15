...
class MovingAverageCrossStrategy(Strategy):
    ...
    def handle_data(self):
        ...
        delDay = nthWeekDay(3, 6, month, year + 2000)
        ...
        ihc, ifc, icc = 'ih%02d%02d.cffex' % (contract_year, contract_month), \
                        'if%02d%02d.cffex' % (contract_year, contract_month), \
                        'ic%02d%02d.cffex' % (contract_year, contract_month)

        ihcOld, ifcOld, iccOld = 'ih%02d%02d.cffex' % (year, month), \
                                 'if%02d%02d.cffex' % (year, month), \
                                 'ic%02d%02d.cffex' % (year, month)

        if (month < contract_month or year < contract_year) and self.secPos[ifcOld] != 0:
            # 需要移仓， 平掉旧合约
...
def run_example():
    ...
    # futures to trade
    ihs = ['ih15%02d.cffex' % i for i in range(5, 13)] + ['ih16%02d.cffex' % i for i in range(1, 2)]
    ifs = ['if15%02d.cffex' % i for i in range(5, 13)] + ['if16%02d.cffex' % i for i in range(1, 2)]
    ics = ['ic15%02d.cffex' % i for i in range(5, 13)] + ['ic16%02d.cffex' % i for i in range(1, 2)]

    futures = ihs + ifs + ics

    universe = indexes + futures
    startDate = dt.datetime(2015, 5, 1)
    endDate = dt.datetime(2015, 12, 28)
