        ...
        **Usage:**
        ...
        for d in os.listdir(rootDirectory):
            ...
            if d.split(".")[-1] in ["md", "mmd", "markdown"]:
                ...
                if currentFM["category"] is None and category is not False and category not in ["projects", "scratch"]:
                    currentFM["category"] = category

                # ADD WIKINAME IF MISSING
                if currentFM["wiki"] is None:
                ...
                count = 1
                if currentFM["tags"]:
                    currentFM["tags"] = str(currentFM["tags"])
                else:
                    currentFM["tags"] = None
                ...
                for t in matchObject:
                    ...
                    if thisTag.lower() not in currentFM["tags"].lower():
        ...
        **Usage:**
        ...
        for d in os.listdir(rootDirectory):
            ...
            if d.split(".")[-1] in ["md", "mmd", "markdown"]:
                ...
                filename = (".").join(
                    d.split(".")[:-1]).lower().replace(" ", "-").replace("'", "")
                title = currentFM["title"]
                if title.lower().strip().replace("  ", " ").replace("  ", " ").replace(" ", "-").replace("'", "") != filename:
                    newfileName = title.lower().strip().replace("  ", " ").replace(
                        "  ", " ").replace(" ", "-").replace("'", "") + ".md"
        ...
        **Usage:**
        ...
        for d in os.listdir(rootDirectory):
            ...
            if d.split(".")[-1] in ["md", "mmd", "markdown"]:
                ...
                if "category" in currentFM and currentFM["category"] and rootFolderCategory in ["projects", "scratch"]:
