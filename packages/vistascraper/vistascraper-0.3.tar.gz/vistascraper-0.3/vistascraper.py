...
def cli(version, url, ip, verbose, quiet, times, continuous, append, out, delay, no_overtype): # Any dashes are converted to underscores here
	...
	# Initialisation
	global stringBuffer # Uses a global because nonlocal isn't Python 2.x compatible, also see echo_temp()
	...
	# Couldn't pass stringBuffer as argument because I want it to function like print(), so uses *args
	# Expects: Anything you can put in 'print()'
	def echo_temp(*args):
		global stringBuffer
		winY = click.get_terminal_size()[1]
		if (ostype == "Windows"): winY = 49 # This seems to be broken in cmd. This is the default...
		if (quiet):
			return
	...
	# Also empties the string buffer
	def flush_buffer():
		global stringBuffer
		if not (no_overtype):
			lines = stringBuffer.count("\n")
		...
		return "" # Empty buffer, be sure to call this function correctly
