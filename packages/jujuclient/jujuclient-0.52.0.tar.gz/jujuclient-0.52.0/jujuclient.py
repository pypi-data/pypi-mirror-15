...
class juju_version(object):
    def __init__(self, juju1_version, juju2_version):
        self.juju1_version = juju1_version
        self.juju2_version = juju2_version

    def __get__(self, obj, objtype):
        if get_juju_major_version() == 1:
            return [self.juju1_version]
        else:
            return [self.juju2_version]
...
class Connector(object):
    ...
    def __init__(self):
        if get_juju_major_version() == 1:
            self.endpoint_segment = "/environment"
            self.parse_env = self.parse_env1
            self.uuid_identifier = "env-uuid"
        else:
            self.endpoint_segment = "/model"
            self.parse_env = self.parse_env2
            self.uuid_identifier = "model-uuid"
    ...
    def connect_env(self, cls, address, name, user, password,
        ...
        if env_uuid:
            endpoint += self.endpoint_segment + "/%s/api" % env_uuid
    ...
    def parse_env2(self, env_name):
        ...
        accounts = self.get_account(jhome, env_name, controller_name)

        try:
            account = accounts['accounts'][accounts['current-account']]

            return jhome, {
                'user': controller['user'],
                'password': account['password'],
                'environ-uuid': model['uuid'],
    ...
    def get_default_model(self, env_name):
        '''Returns the default juju2 controller:model.'''
    ...
    def get_controller(self, env_name, controller_name=None):
        '''Returns info for the specified or current controller.'''
        try:
    ...
    def get_model(self, env_name, controller_name, model_name):
        '''Returns info for the specified or current model.'''
        model = '{}:{}'.format(controller_name, model_name)
        try:
            output = subprocess.check_output(
                ['juju', 'get-model-config', '-m', model, '--format=yaml'])
        except subprocess.CalledProcessError:
            raise EnvironmentNotBootstrapped(env_name)
        return yaml.safe_load(output)

    def get_account(self, jhome, env_name, controller_name):
        '''Returns user info for the specified controller.'''
        ...
        with open(cache_filename) as fh:
            ...
            try:
                ...
                return {
                    ...
                    'environ-uuid': environment[self.uuid_identifier],
...
class Environment(RPC):
    ...
    def __init__(self, endpoint, name=None, conn=None,
        ...
        # Instantiate our service object for juju 1, 2 compatibility
        if get_juju_major_version() == 1:
            self.endpoint_segment = "/environment"
            self.version = 0
            self.service = Service1(self)
            self.annotation = ClientAnnotation1(self)
            self.tag_prefix = "environment"
            self.info_request = "EnvironmentInfo"
            self.get_constraints_request = "GetEnvironmentConstraints"
            self.set_constraints_request = "SetEnvironmentConstraints"
            self.get_request = "EnvironmentGet"
            self.set_request = "EnvironmentSet"
            self.unset_request = "EnvironmentUnset"
            self.agent_version_request = "SetEnvironAgentVersion"
            self.juju_home = os.environ.get('JUJU_HOME', '~/.juju')
        else:
            self.endpoint_segment = "/model"
            self.version = 1
            self.service = Service2(self)
            self.annotation = ClientAnnotation2(self)
            self.tag_prefix = "model"
            self.info_request = "ModelInfo"
            self.get_constraints_request = "GetModelConstraints"
            self.set_constraints_request = "SetModelConstraints"
            self.get_request = "ModelGet"
            self.set_request = "ModelSet"
            self.unset_request = "ModelUnset"
            self.agent_version_request = "SetModelAgentVersion"
            self.juju_home = os.environ.get('JUJU_DATA', '~/.local/share/juju')
    ...
    @property
    def tag(self):
        return "%s-%s" % (self.tag_prefix, self._env_uuid)
    ...
    def _http_conn(self):
        ...
        path = ""
        if self._env_uuid:
            path = self.endpoint_segment + "/%s" % self._env_uuid
    ...
    def get_env_constraints(self):
        ...
        return self._rpc({
            "Type": "Client",
            "Request": self.get_constraints_request})
    ...
    def set_env_constraints(self, constraints):
        ...
        return self._rpc({
            "Type": "Client",
            "Request": self.set_constraints_request,
            "Params": {
                "ServiceName": "",
                "Constraints": self.env._prepare_constraints(constraints)}})
    ...
    def set_env_agent_version(self, version):
        ...
        return self._rpc({
            "Type": "Client",
            "Request": self.agent_version_request,
    ...
    def get_annotation(self, *args, **kws):
        return self.annotation.get_annotation(*args, **kws)

    def set_annotation(self, *args, **kws):
        return self.annotation.set_annotation(*args, **kws)
...
def negotiate_facades(env, server_info):
    ...
    facades = {}
    # juju1 - Facades, juju2 - facades
    for fenv in server_info.get('Facades') or server_info.get('facades') or []:
...
class ClientAnnotation1(APIFacade):
    ...
    def set_annotation(self, entity, entity_type, annotation):
        ...
        """
        # valid entity types
        a = self.env._prepare_strparams(annotation)
        return self.rpc({
            "Type": "Client",
...
class ClientAnnotation2(APIFacade):
    """Access for annotations.
    """
    key = "client"
    name = "Client"
    ...
    def __init__(self, client, version=None):
        self.annotation = Annotations(client)
    ...
    def ensure_availability(self, num_state_servers, series=None,
        ...
        if get_juju_major_version() == 1:
            ...
            return self.rpc({
                ...
                'Params': {
                    'Specs': [{
                        'ModelTag': "model-%s" % self.env._env_uuid,
                        'NumControllers': int(num_state_servers),
                        'Series': series,
                        'Constraints': self._format_constraints(constraints),
                        'Placement': placement}]}})
...
class Service1(APIFacade):
    ...
    def deploy(self, service_name, charm_url, num_units=1,
        ...
        svc_config = {}
        if config:
            svc_config = self.env._prepare_strparams(config)

        svc_constraints = {}
        if constraints:
            svc_constraints = self.env._prepare_constraints(constraints)
    ...
    def update_service(self, service_name, charm_url="", force_charm_url=False,
        ...
        svc_config = {}
        if settings:
            svc_config = self.env._prepare_strparams(settings)
...
class Service2(APIFacade):
    ...
    def deploy(self, service_name, charm_url, num_units=1,
        ...
        svc_placement = []
        if machine_spec:
            svc_placement = [self.env._prepare_placement(machine_spec)]
...
def EnvironmentManager():
    if get_juju_major_version() == 1:
        return EnvironmentManager1()
    else:
        return EnvironmentManager2()


class EnvironmentManager1(APIFacade):
    ...
    ***Jan 2015 - Note MESS is still under heavy development
    ...
    def __init__(self, client, version=None):
        self.create_request = 'CreateEnvironment'
        self.list_request = 'ListEnvironments'
...
class EnvironmentManager2(EnvironmentManager1):
    key = "modelmanager"
    name = "ModelManager"
    versions = [2]

    def __init__(self):
        self.create_request = 'CreateModel'
        self.list_request = 'ListModels'
...
class WaitForNoMachines(WatchWrapper):
    ...
    def complete(self):
        if list(self.machines.keys()) == ['0'] and self.watch.version == 0:
            return True
