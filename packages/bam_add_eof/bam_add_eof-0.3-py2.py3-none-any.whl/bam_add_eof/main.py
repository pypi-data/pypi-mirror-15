...
def add_entry_rebam_table(uuid, bam_name, engine, logger):
    df = pd.DataFrame({'uuid': [uuid],
                       'bam_name': bam_name
    })
    unique_key_dict = {'uuid': uuid, 'bam_name': bam_name}
    table_name = 'bam_missing_eof'
    df_util.save_df_to_sqlalchemy(df, unique_key_dict, table_name, engine, logger)
    return

def add_entry_no_rebam_table(uuid, bam_name, engine, logger):
    df = pd.DataFrame({'uuid': [uuid],
                       'bam_name': bam_name
    })
    unique_key_dict = {'uuid': uuid, 'bam_name': bam_name}
    table_name = 'bam_not_missing_eof'
    df_util.save_df_to_sqlalchemy(df, unique_key_dict, table_name, engine, logger)
    return
...
def main():
    ...
    if missing_eof(bam_path, picard_validation_path, engine, logger):
        ...
        add_entry_rebam_table(uuid, bam_name, engine, logger)
    else:
        logger.info('performing copyfile() of %s as has EOF' % bam_name)
        shutil.copyfile(bam_path, bam_name)
        add_entry_no_rebam_table(uuid, bam_name, engine, logger)
    return
