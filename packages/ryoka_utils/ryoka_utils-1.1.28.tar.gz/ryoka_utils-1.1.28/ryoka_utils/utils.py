    ...
    def get_pid(self, keyword):
        ...
        for line in lines:
            ...
            for idx in range(len(l)):
                if (l[idx].find(keyword) < 0): continue
                pid_list.append(int(l[pid_pos]))
        return list(set(pid_list))

    def _unzip_tar(self, src, dst):
        tf = open(src)
        tf.extractall(dst)
        tf.close()

    def _zip_tar(self, src, dst, mode):
        tf = open(dst, mode)
