...
def input_parse(infile):
    if "(" in infile:
        ...
        if ")" in infile[i:]:
            ...
            return infile[:i], slice(start, stop, step)
        else:
...
def pcazip(args):
    ...
    if args.tests:
        ...
        try:
            ...
            os.chdir(tdir)
            call(['wget','-q','-P', tdir,
                     'https://bitbucket.org/ramonbsc/pypcazip/downloads/test153.tar.gz'])
            call(['tar', 'xf', tdir + '/test153.tar.gz', "-C", tdir])
            os.chdir(tdir + '/test')
            call(['./run_tests.sh'])
            os.chdir(here)
            if args.verbosity:
    ...
    for a in uniStr:
        ...
        if op.splitext(a[1])[1].lower() == '.ncdf' and nonetCDF4:
            ...
            exit(1)
        f.append(Fasu(a[0], a[1], frames=a[2], selection=a[3], **kwargs))
        i += 1
    try:
    ...
    if args.nopca is False:
        ...
        time2start = time()
        p = Pcz(cf, quality=float(args.quality), req_evecs=args.evecs,
        ...
        if rank == 0:
            ...
            p.write(output_file, version=args.file_version)
        ...
        if rank == 0:
            ...
            log.info('Time for reading input files: {0:.2f} s, {1:.1f}% total runtime\n'.format(time1end - time1start, (
