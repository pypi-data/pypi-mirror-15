...
__all__ = ["config","deployment_package","lambda_function"]

Api = {}

def lambda_entry_point(entry_point):
    print("Defining entry point: %s" % entry_point.__name__)
    lambda_function.LambdaFunction(entry_point.__name__, entry_point.__module__)
    return entry_point
...
def api_method(verb="GET", path="/"):
    ...
    def install_api_method(func):
        ...
        return func
    print("Defining API method: %s") % key
