...
class Client():
    ...
    def __get__(self, uri, data={}, verbose={}):
        endpoint = "%s/%s" % (self.base_url, uri)
    ...
    def security(self, repository, imageid):
        ...
        response = self.__get__(uri)

        return json.loads(response)

    def get_images_by_tag(self, repository, tag):
        uri = 'repository/%s/tag/%s/images' % (repository, tag)
        response = self.__get__(uri)
