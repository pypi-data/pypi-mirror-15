...
class ReviewResponse(object):
    ...
    def review(self):
        for rule in self.rules:
            ...
            if res:
                self.code = rule[RESULT_CODE]
                self.reason = rule[RESULT_REASON]
                logger.debug("MATCHED: code: {}, reason{}".format(
...
def review_response(resp, rules=None):
    ...
    Returns:
        A ReviewResponse object, containing:
            ...
            .code: the "corrected" HTTP code. Will be the same as the original
                request if no corrections were made, or will be a new value
                if updated.
