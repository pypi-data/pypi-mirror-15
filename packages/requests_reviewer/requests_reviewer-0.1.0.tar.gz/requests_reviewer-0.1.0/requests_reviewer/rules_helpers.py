...
def match_content(r, target_str):
    logger.debug(u"Content matching: {} in {}".format(target_str, r.text))
...
def match_url(r, url_str):
    logger.debug(u"URL matching: {} in {}".format(url_str, r.url))
    return r.url.find(url_str) >= 0


def match_header(r, header_name, header_val):
    logger.debug(u"HTTP header matching: {}: {} in {}".format(
...
def match_exact_header(r, header_name, header_val):
    logger.debug(u"HTTP header exact matching: {}: {} in {}".format(
        header_name, header_val, r.headers))
