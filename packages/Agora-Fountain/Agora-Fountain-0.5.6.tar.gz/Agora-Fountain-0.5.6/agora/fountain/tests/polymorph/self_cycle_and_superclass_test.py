...
class SelfCycleAndSuperclassGraphTest(FountainTest):
    def test_graph(self):
        self.post_vocabulary('one_superclass')
        self.post_vocabulary('self_cycle')

        expected_graph = AgoraGraph()
        expected_graph.add_types_from(['test:Concept1', 'test:SuperConcept1'])
        expected_graph['test:SuperConcept1']['sub'] = ['test:Concept1']
        expected_graph['test:Concept1']['super'] = ['test:SuperConcept1']
...
class SelfCycleAndSuperclassSuperPathsTest(FountainTest):
    def test_path(self):
        self.post_vocabulary('one_superclass')
        self.post_vocabulary('self_cycle')
        self.post_seed("test:Concept1", seed_uri)
        paths, all_cycles = self.get_paths("test:SuperConcept1")
...
class SelfCycleAndSuperclassSelfPathsTest(FountainTest):
    def test_path(self):
        self.post_vocabulary('one_superclass')
        self.post_vocabulary('self_cycle')
        self.post_seed("test:Concept1", seed_uri)
