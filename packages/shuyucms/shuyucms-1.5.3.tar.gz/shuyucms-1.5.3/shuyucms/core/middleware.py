...
from shuyucms.utils.cache import (cache_key_prefix, nevercache_token, _hashed_key,
...
class _Deprecated(object):
    def __init__(self, *args, **kwargs):
        ...
        msg = "shuyucms.core.middleware.%s is deprecated." % self.old
        if self.new:
            msg += (" Please change the MIDDLEWARE_CLASSES setting to use "
                    "shuyucms.core.middleware.%s" % self.new)
        warn(msg)
...
class AdminLoginInterfaceSelectorMiddleware(object):
    ...
    def process_view(self, request, view_func, view_args, view_kwargs):
        login_type = request.POST.get("shuyucms_login_interface")
...
class FetchFromCacheMiddleware(object):
    ...
    def process_request(self, request):
        if cache_installed() and request.method == "GET":
            ...
            # 查看某个页面的 cache_key
            # debug_file = open('log/debug.log', 'a+')
            # debug_file.write(str(cache_key) + str('\n'))
            # debug_file.write(str(_hashed_key(cache_key)) + str('\n'))
            # debug_file.close()
    ...
    def process_request(self, request):
        ...
        if response and request.method == "POST":
            if resolve(request.get_full_path()).url_name == "fb_do_upload":
                # The handler for the flash file uploader in syfilebrowser
                ...
                # when /admin is restricted to SSL using shuyucms's SSL
