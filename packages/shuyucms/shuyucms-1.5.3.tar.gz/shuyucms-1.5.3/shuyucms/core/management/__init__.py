...
from shuyucms.utils.tests import copy_test_to_media
...
def is_full_install():
    for app in ["forms", "galleries", "blog", "pages"]:
        if "shuyucms.%s" % app not in settings.INSTALLED_APPS:
            return False
    return True
...
def create_pages(app, created_models, verbosity, interactive, **kwargs):
    ...
    from shuyucms.forms.models import Form
    from shuyucms.galleries.models import Gallery

    required = set([Page, Form, Gallery])
    if required.issubset(set(created_models)):
        call_command("loaddata", "shuyucms_required.json")
        if interactive:
...
def install_optional_data(verbosity):
    ...
    if verbosity >= 1:
        ...
        print()
    from shuyucms.galleries.models import Gallery
    call_command("loaddata", "shuyucms_optional.json")
    zip_name = "gallery.zip"
    copy_test_to_media("shuyucms.core", zip_name)
...
if not settings.TESTING:
    ...
    if "shuyucms.pages" in settings.INSTALLED_APPS:
        from shuyucms.pages import models as pages_app
