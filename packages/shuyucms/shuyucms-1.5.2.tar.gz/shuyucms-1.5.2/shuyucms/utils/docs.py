...
def build_deploy_docs(docs_path):
    try:
        ...
        return
    project_template_path = path_for_import("shuyucms.project_template")
...
def build_changelog(docs_path, package_name="shuyucms"):
    """
...
def build_modelgraph(docs_path, package_name="shuyucms"):
    """
    ...
    apps = [a.rsplit(".")[1] for a in settings.INSTALLED_APPS
            if a.startswith("shuyucms.") or a.startswith(package_name + ".")]
    try:
...
def build_requirements(docs_path, package_name="shuyucms"):
    """
    Updates the requirements file with shuyucms's version number.
    """
    mezz_string = "shuyucms=="
    project_path = os.path.join(docs_path, "..")
