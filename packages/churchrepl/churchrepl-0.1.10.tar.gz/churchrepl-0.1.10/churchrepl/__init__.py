...
def repl(prompt, args):
    ...
    def add(body, func):
        ...
        :param body: the lambda body
        :param func: the lambda itself
        :return: _add(x, a) or 0
        """
        return _add(body, func) if body is not None else 0

    def toint(lambda_expr):
        """
        Church enumeration converter
        :param lambda_expr: the lambda to convert
        :return: x's enumerated value.
        """
        return lambda_expr + 1 if isinstance(lambda_expr, int) else add
    ...
    if args.file is not None and len(args.file) is not 0:
        ...
        if ast is not None:
            try:
                ...
                i = 0
                for i in defs:
                    place(i, defs[i].evalstr())
                for j in [eval(lexical.evalstr)(toint)(0)
                          for lexical in lexes]:
                    place(str(j), str(lexes[i].evalstr()))
                    i += 1
