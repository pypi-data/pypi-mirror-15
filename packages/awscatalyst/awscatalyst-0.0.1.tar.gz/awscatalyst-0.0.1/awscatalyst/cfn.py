...
def cfn_resource(cfn_r):
    """
    Helper for boto3 resource object to AWS CloudFormation.
    Create new one (to default region) if cfn_r is Falsy, return cfn_r otherwise.

    :param cfn_r: reference to AWS CFN
    :return: boto3 CloudFormation resource or cfn_r
    """
    import boto3
    return cfn_r or boto3.resource('cloudformation')
...
class CfnStack(object):
    ...
    def list(cfn=None):
        ...
        :rtype: list(Stack)
        """
        for stack in cfn_resource(cfn).stacks.all():
            yield stack
