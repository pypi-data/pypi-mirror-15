...
def get_mapped_document(mappings, document, namespace):
    ...
    for key in get_scalar_array_fields(mappings, db, collection, document):
        flat_document[key] = to_scalar_string(document[key])
...
def get_scalar_array_fields(mappings, db, collection, document):
    if db not in mappings or collection not in mappings[db]:
        return []

    return [
        k for k, v in mappings[db][collection].iteritems()
        if k in document and 'type' in v and v['type'] == '_ARRAY_OF_SCALARS'
        ]
