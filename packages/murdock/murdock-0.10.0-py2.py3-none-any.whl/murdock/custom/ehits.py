...
"""
Module :mod:`murdock.custom.ehits`
----------------------------------

A wrapper module for the software eHiTS. It provides the classes
:class:`Search` and :class:`Scoring` which are conform with the Murdock
:mod:`~murdock.search` and :mod:`~murdock.scoring` APIs.
...
log = logging.getLogger('custom.ehits')
...
class Scoring(murdock.scoring.Scoring):
    ...
    def setup(self, parms, docking, name='eHiTS', copy=False):
        ...
        self.name = name
        #: :class:`~murdock.runner.docking.Docking` instance associated with
        #: this scoring
    ...
    def _dock(self):
        ...
        for lignode, ligpath in ligands.iteritems():
    ...
    def _match_and_overwrite(self, lignode, molstruct):
        matches = murdock.molstruct.match_atoms(
            ...
            match_name=True)
        missing_in_old = [
            _a for _a in molstruct.atoms() if _a not in matches.itervalues()]
        missing_in_docked = [
            _a for _a in lignode.atoms if _a not in matches.iterkeys()]
            ...
            if missing_in_docked:
                log.error(
                    ...
                    ', '.join([
                        ...
                        missing_in_docked]))
            return False
        for old_atom, docked_atom in matches.iteritems():
...
class ConfigDeclaration(murdock.config.ConfigDeclaration):
    ...
    def parameters(self):
        ...
        opt.append(Option(
            name='interpreter', dtype=basestring, description='filepath to '
            'shell interpreter', check_filepath=True))
        opt.append(Option(
            name='workdirname', dtype=basestring, description='name of '
            'working directory to be created', default='ehits_work'))
        opt.append(Option(
            name='ehits_executable', dtype=basestring, description='path to '
            'the external eHiTS executable', check_filepath=True))
        opt.append(Option(
            name='accuracy', dtype=int_gt_zero, description='eHiTS '
            'command-line parameter `-accuracy`'))
        opt.append(Option(
            name='blind', dtype=bool, default=False, description='whether to '
            'perform a blind docking'))
        opt.append(Option(
            name='margin', dtype=int_gt_zero, description='eHiTS command-line '
            'parameter `-margin`', default=10.0))
        opt.append(Option(
            name='tune_parameters', dtype=basestring, required=False,
