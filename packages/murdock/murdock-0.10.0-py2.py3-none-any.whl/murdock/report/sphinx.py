...
"""
This module handles SPHINX projects (based on reStructuredText.
...
log = logging.getLogger('report.sphinx')
...
class Document(murdock.report.Document):
    ...
    def add_figure(
            ...
            scale=None):
        if True in (
                _v == self.fmt_figure(label) for _v in
                self._subs.itervalues()):
...
class ReSTTable(murdock.report.Table):
    ...
    def create(self):
        ...
        t += '%s\n' % self._get_subhline()
        try:
            t += '%s\n' % ' '.join(self.data.iterkeys())
            t += '%s\n' % self._get_hline()
        except TypeError:
            pass
        t += '%s\n' % ' '.join(
            _shead for _stable in self.data.itervalues() for _shead in
            _stable.iterkeys())
        t += '%s\n' % self._get_subhline()
        # Add table rows.
        i = 0
        while True:
            try:
                t += '%s\n' % ' '.join(
                    _col[i] for _stable in self.data.itervalues() for _col in
                    _stable.itervalues())
            except IndexError:
                break
            i += 1
    ...
    def _get_hline(self):
        return ' '.join('-' * (len(_head)) for _head in self.data.iterkeys())

    def _get_subhline(self):
        return ' '.join([
            '=' * len(_shead) for _stable in self.data.itervalues() for _shead
            in _stable.iterkeys()])
