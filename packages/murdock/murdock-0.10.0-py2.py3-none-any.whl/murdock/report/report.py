...
"""
Module :mod:`murdock.report.report`
-----------------------------------

This module creates human-readable reports using different backends. It
provides the base classes :class:`Report` and :class:`RuntimeReport` as well as
an implementation for each Murdock runner: :class:`DockingReport`,
:class:`ScreeningReport` and `TrainingReport`.

In addition, it specifies the report API by providing the base classes
:class:`Project` and :class:`Document`. A list of implemented report backends
is given :mod:`here <murdock.report>`.
...
try:
    ...
    MATPLOTLIB = False
    MATPLOTLIB_ERROR = traceback.format_exc()
...
if MATPLOTLIB:
    ...
    log.warning(
        'Error during import of `matplotlib` or related package: %s',
        MATPLOTLIB_ERROR)
    log.warning('Result plots will not be created.')
...
class Project(object):
    ...
    def _get_toc_docs(self):
        return collections.OrderedDict([
            (_label, _doc) for _label, _doc in self.documents.iteritems() if
...
class Document(object):
    ...
    def __init__(
        ...
        #: class:`Project` instance this Document belongs to
        self.project = project
        #: content of output file
        self._output = ''
        #: current level of indentation
        self._indent = ''
        #: dictionary of substitution to be performed in :meth:`substitute()`
    ...
    def substitute(self):
        """Substitute all uuid_keys (:attr:`subs`) in :attr:`output`.
        """
...
class RunnerReport(Report):
    ...
    def _add_docked_pictures(
        ...
        for step in self.res.steps:
            ...
            for run in step.iter_samples(status='finished'):
                ...
                for fp in run.filepaths.itervalues():
            ...
            res_colors = collections.OrderedDict(
                ...
                for _r, _val in topres.iteritems())
    ...
    def _add_averages_table(self):
        ...
        for step in self.res.steps:
            ...
            for measure, quality in step.qualities.iteritems():
                for mtype in ('receptor', 'ligand'):
                    vals = [
                        _val for _mol, _vals in quality.iteritems() if
    ...
    def _add_quality_plots(
        ...
        thead = '%s distributions (averages)' % '/'.join(row[None].iterkeys())
    ...
    def _add_rotbond_plots(
        ...
        for step in self.res.steps:
            ...
            steprbs = collections.OrderedDict((
                ...
                _mol in step.rotbonds.iterkeys()))
            refrbs = None if not step.reference else collections.OrderedDict((
                (_mol, step.rotbonds.get_bond_dict(_mol, norm=norm)) for
                _mol in step.reference.rotbonds.iterkeys()))
            for mol, moldat in steprbs.iteritems():
                data = collections.OrderedDict()
                data['docked'] = moldat
    ...
    def _add_timetable(self):
        ...
        if not td:
            return False
        for ttype in rows.iterkeys():
            rows[ttype]['Total']['Elapsed'] = td.formatted(ttype)
        extra_tds = self.res.timing_extra()
        for ttype in rows.iterkeys():
            for exlabel, extd in extra_tds.iteritems():
                rows[ttype]['Total'][exlabel] = extd.formatted(ttype)
        for ttype, fmt_time in self.res.timing_fmt_mean(std=True).iteritems():
            rows[ttype]['Averages']['Run'] = fmt_time
        for step in self.res.steps:
            for ttype, fmt_time in step.timing_fmt_mean(std=True).iteritems():
        ...
        if self.res.status != 'finished':
            ...
            for shead, tdict in estimate_td.iteritems():
                for ttype, fmt_time in tdict.iteritems():
                    rows[ttype]['Estimate'][shead] = fmt_time
        self.add_table(rows.values(), 'Running Time')
        return True
    ...
    def _add_residue_plots(
        ...
        for step in self.res.steps:
            ...
            if not ress:
                continue
            ress = collections.OrderedDict(ress.items()[:nres])
            if isinstance(self, DockingReport):
                try:
                    ...
                    refs = collections.OrderedDict(
                        (_name, refs[_name]) for _name in ress.iterkeys() if
                        _name in refs)
            elif isinstance(self, ScreeningReport):
                refs = None
            else:
...
class DockingReport(RunnerReport):
    ...
    def _add_cluster_table(
        ...
        rows = [_row for _row in rowdict.itervalues() if len(_row) > 1]
    ...
    def _add_result_table(self):
        ...
        for run in self.res.runs:
            for step in run.iter_steps(status='finished'):
                ...
                for measure, quality in step.qualities.iteritems():
                    for mol, molval in quality.iteritems():
                        if molval is None:
                            continue
                        if quality.rank == 1:
                ...
                if MATPLOTLIB and True in self.draw_res_charts.itervalues():
    ...
    def _add_search_plots(
        ...
        for step in self.res.steps:
            ...
            for tname, tvals in scores.iteritems():
                try:
                    row = rows[tname]
                except KeyError:
                ...
                shead = 'Step %d (%s)' % (step.serial, step.title)
                row[None][shead] = link
        self.add_table(rows.values(), 'Convergence of Scores')
        return True
    ...
    def _get_residue_charts(self, step, label, force=False, width=1000):
        ...
        for measure, plot_class in plot_classes.iteritems():
            ...
            try:
                ...
                continue
            for mol, moldata in contr.iteritems():
...
class ScreeningReport(RunnerReport):
    ...
    def _add_progresstext(self):
        ...
        if stats:
            self.add_bullet_list(
                collections.OrderedDict(sorted(stats.items())))
        else:
            self.add_paragraph(self.res.status)
        return True
    ...
    def _add_quality_dockwise_plots(
            ...
            for step in self.res.steps:
                ...
                for quality in step.qualities[measure].qualities:
                    ...
                    if dres.title not in data['all']:
                        data['all'][dres.title] = []
                    data['all'][dres.title].extend(quality.values())
                    if quality.step.scoring.rank == 1:
                        data['top-scored'][dres.title] = quality.values()
                    if quality.rank == 1:
                        data['best'][dres.title] = quality.values()
        ...
        thead = '%s distributions (dockings)' % '/'.join(row.iterkeys())
...
class Table(object):
    ...
    def add_row(self, row):
        ...
        if self.rows and None not in row and None in self._get_heads():
            raise ReportError(
                ...
                '%s' % str(row))
        if False in (
                isinstance(_field, basestring) for _stable in row.itervalues()
                for _field in _stable.itervalues()):
            raise ReportError(
                'Row can not be added because it contains non-string data: %s'
                % str(row))
        rows = row.splitlines()
    ...
    def _fill_table(self):
        for head, subtable in self.data.iteritems():
            for shead, column in subtable.iteritems():
    ...
    def _get_column_widths(self):
        ...
        for head, subtable in self.data.iteritems():
            ...
            widths[head] = (
                sum(swidths[head].itervalues()) + len(swidths[head]) - 1)
            if head is not None and len(head) > widths[head]:
                add_headspace = max(0, len(head) - widths[head])
                swidths[head][swidths[head].keys()[-1]] += add_headspace
    ...
    def _get_heads(self):
        ...
        return list(collections.OrderedDict.fromkeys(
            _k for _row in self.rows for _k in _row.iterkeys()))
    ...
    def _init_table_structure(self):
        ...
        for head in heads:
            ...
            for row in self.rows:
                if head not in row:
                    continue
                for shead in row[head].iterkeys():
                    if shead not in self.data[head]:
                        self.data[head][shead] = []
        if len(heads) == 1 and len(self.data.values()[0]) == 1:
            self.data[heads[0]][' '] = []
        return True
...
class TableRow(collections.OrderedDict):
    ...
    def remove_empty_fields(self):
        for head, stable in self.iteritems():
            for shead, field in stable.iteritems():
                if not field:
                    del self[head][shead]
            if not stable:
                del self[head]
    ...
    def splitlines(self):
        split = collections.OrderedDict((
            _head, collections.OrderedDict(
                ...
                _stable.iteritems())) for _head, _stable in self.iteritems())
        height = max(
            len(_field) for _stable in split.itervalues() for _field in
            _stable.itervalues())
        return [
            TableRow(
                (
                    _head, collections.OrderedDict(
                        ...
                        _shead, _lines in _stable.iteritems())
                ) for _head, _stable in split.iteritems()) for _i in
            xrange(height)]
...
def substitute_fields(data, subdict):
    ...
    for head, subtable in data.iteritems():
...
def substitute_all(text, sdict):
    if not sdict or not isinstance(text, basestring):
        return text
    pattern = re.compile(r'|'.join([re.escape(_k) for _k in sdict.iterkeys()]))
