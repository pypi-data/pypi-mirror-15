...
PyObject * LineWidth(PyObject * self, PyObject * args) {
	float size;

	if (!PyArg_ParseTuple(args, "f:LineWidth", &size)) {
		return 0;
	}

	ModernGL::LineWidth(size);
	Py_RETURN_NONE;
}
...
PyObject * AttribLocation(PyObject * self, PyObject * args) {
	unsigned program;
	const char * name;

	if (!PyArg_ParseTuple(args, "Is:AttribLocation", &program, &name)) {
		return 0;
	}

	return PyLong_FromLong(ModernGL::AttribLocation(program, name));
}
...
static PyMethodDef methods[] = {
	...
	{"LineWidth", LineWidth, METH_VARARGS, 0},
	...
	{"AttribLocation", AttribLocation, METH_VARARGS, 0},
...
static struct PyModuleDef moduledef = {
	PyModuleDef_HEAD_INIT, "PyModernGL", 0, -1,
	methods, 0, 0, 0, 0
};

extern "C" {
	PyObject * PyInit_PyModernGL();
}
