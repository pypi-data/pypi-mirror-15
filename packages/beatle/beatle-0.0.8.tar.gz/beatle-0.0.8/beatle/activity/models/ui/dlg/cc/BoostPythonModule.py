...
class BoostPythonModuleDialog(ui.NewBoostPythonModule):
    ...
    def append_classes(self, pid, container):
        ...
        for cls in container(model.cc.Class, filter=f, cut=True):
            item = self.m_tree.AppendItem(pid, cls.name, ct_type=1, data=cls)
            self._eert[cls] = item
    ...
    def append_functions(self, pid, container):
        ...
        for fn in container(model.cc.Function):
            item = self.m_tree.AppendItem(pid, fn.name, ct_type=1, data=fn)
            self._eert[fn] = item
    ...
    def get_kwargs(self):
        ...
        # pending content += self.scan_methods(self._project)
        # Then, create a module
        model.cc.pyBoostModule(
