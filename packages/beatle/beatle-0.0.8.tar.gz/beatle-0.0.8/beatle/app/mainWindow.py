...
class mainWindow(base.MainWindow):
    ...
    def _import_tools(self):
        ...
        for tool in self._tools:
            self.PushEventHandler(tool.load())
    ...
    def SaveViews(self):
        ...
        for v in activity.ALL:
            config.Write('/views/perspectives/{}'.format(v.name), v.perspective)
            if v in self._views and self._views[v]:
                config.WriteInt('/views/states/{}'.format(v.name), 1)
            else:
                config.WriteInt('/views/states/{}'.format(v.name), 0)
    ...
    def LoadViews(self):
        ...
        for v in activity.ALL:
            v.perspective = config.Read('/views/perspectives/{}'.format(v.name), '')
            if config.ReadInt('/views/states/{}'.format(v.name), 1):
    ...
    def OnEditUndo(self, event):
        ...
        wx.LogMessage('undo {}'.format(tran.TransactionStack.instance.UndoName()))
    ...
    def OnEditRedo(self, event):
        ...
        wx.LogMessage('redo {}'.format(tran.TransactionStack.instance.RedoName()))
    ...
    def Destroy(self):
        ...
        for view in views:
            views[view]._delete_toolbars()
        super(mainWindow, self).Destroy()

    @tran.TransactionalMethod('create workspace {}')
    ...
    def OnEraseStartPage(self, event):
        ...
        bmp.LoadFile('app/res/beatle.xpm', wx.BITMAP_TYPE_XPM)
        dc.DrawBitmap(bmp, sz.x // 2 - 400, sz.y // 2 - 300)
        del bmp
    ...
    def OnEditPreferences(self, event):
        ...
        dlg.PreferencesDialog(self).ShowModal()
    ...
    @tran.TransactionalMethod('open workspace {}')
    def OnOpenWorkspace(self, event):
        ...
        if dialog.ShowModal() != wx.ID_OK:
            return False
        obj = context.App().LoadWorkspace(dialog.GetPath())
