...
class Editor(StyledTextCtrl):
    """Common C++ editor class"""
    def __init__(self, parent, id=wx.ID_ANY, language='c++', bookmarks={}, breakpoints={}):
        """Initialize control"""
        ...
        #Default font style
        config = ctx.localcontext.Config()
        ...
        if self._language == 'c++':
            ...
            self.StyleSetSpec(STC_P_NUMBER, "fore:#009600,back:#FFFFFF,bold")
    ...
    def OnKey(self, event):
        ...
        caretPos = self.GetCurrentPos()
        if caretPos > 2:
            self.GotoPos(caretPos)
            self.SearchAnchor()
            pos = self.SearchPrev(STC_FIND_REGEXP,
                r'[ \t\n^]')
            self.GotoPos(caretPos)
            if pos == wx.NOT_FOUND:
                return
            pos = pos + 1
            s = self.GetTextRange(pos, caretPos)
            match = re.search(r'[ \t\n^](?P<rest>.*)', s)
            if match:
                pos = pos + len(s) - len(match.group('rest'))
                s = match.group('rest')
            # pending : this is not correct, but sended key dont get shift
            if self.popup_handler and k in [ord('.'), ord('>'), ord('<')]:
                ...
                if s[-2:] == '->':
                    ...
                    return
            if caretPos - pos > 2:
                self.AutoCompShow(caretPos - pos, self._auto)
