...
class NavigatorDialog(ui.Navigator):
    ...
    def FillTree(self, treeid, obj, prefix=wx.EmptyString):
        ...
        if type(obj) is cc.Class:
            ...
            for member in obj(cc.MemberData, filter=f, cut=True):
                treeitemid = self.m_tree.AppendItem(treeid, '{prefix}{name}'.format(
                    prefix=prefix, name=member.name), image=member.bitmap_index)
            ...
            for function in obj(cc.MemberMethod, filter=f, cut=True):
                treeitemid = self.m_tree.AppendItem(treeid, '{prefix}{call}'.format(
                    prefix=prefix, call=function.call), image=function.bitmap_index)
                if type(function.type_instance.base_type) is cc.Class:
                    # Add has child property
                    self.m_tree.SetItemHasChildren(treeitemid, True)
                self.m_tree.SetPyData(treeitemid, wx.TreeItemData(function))
    ...
    # Handlers for Navigator events.
    def OnSelectElement(self, event):
        # TODO: Implement OnSelectElement
        pass
