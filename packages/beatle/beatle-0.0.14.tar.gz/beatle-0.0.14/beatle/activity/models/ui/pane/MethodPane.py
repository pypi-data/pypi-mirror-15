...
class MethodPane(ui.MethodPane):
    ...
    # command ids:
    _saveCode = wx.Window.NewControlId()
    _undoCode = wx.Window.NewControlId()
    _redoCode = wx.Window.NewControlId()
    _copyCode = wx.Window.NewControlId()
    _cutCode = wx.Window.NewControlId()
    _pasteCode = wx.Window.NewControlId()
    _deleteCode = wx.Window.NewControlId()
    _toggleFull = wx.Window.NewControlId()
    _navigator = wx.Window.NewControlId()
    ...
    def __init__(self, parent, mainframe, method):
        ...
        self.m_editor.SetText(method._content)
        ...
        self.m_editor.SetReadOnly(method._readOnly)
    ...
    def __analize(self):
        """Do a quick code analysis for determining variables"""
        self._vars = {}
        self._vars.update([(x.name, x._typei) for x in self._object[cc.Argument]])
        pos = self.m_editor.GetCurrentPos()
        text = self.m_editor.GetText()[:pos]
        # remove some extra code
        remove_delimiters = [
            ('/*','*/'),
            ('{','}')
        ]
        for delimiter in remove_delimiters:
            test = True
            while test:
                test = False
                left_pos = text.rfind(delimiter[1])
                if left_pos >= 0:
                    right_pos = text.rfind(delimiter[0],0,left_pos)
                    if right_pos < 0:
                        # the currend code seems incorrect
                        return False
                    text = text[:right_pos] + text[left_pos:]
                    test = True
        #use regexpr for possible var declarations
        name = r'[_A-Za-z]+[_A-Za-z0-9]*'
        var = r'[\*&]?{name}'.format(name=name)
        _type =r'{name}(::{name})*'.format(name=name)
        _vlist=r'{var}(\s*,\s*{var})*'.format(var=var)
        decl = re.compile(r'(?P<type>{_type})\s+(?P<varlist>{_vlist})\s*;'.format(
            _type=_type, _vlist=_vlist))
        clean = lambda x : x.replace('*','').replace('&','').strip()
        for match in decl.finditer(text):
            t = match.group('type')
            v = [clean(x) for x in match.group('varlist').split(',')]
            if t in self._types:
                k = self._types[t]
                self._vars.update([(x,k) for x in v])
        return True
    ...
    def Popup(self, editor, text=None):
        ...
        if text:
            ...
            if not self.__analize():
                return
            if text not in self._vars:
                return
            v = self._vars[text]
            if type(v) is cc.Class:
                pos = editor.GetCurrentPos()
                p = editor.PointFromPosition(pos)
                p = editor.ClientToScreen(p)
                dlg = NavigatorDialog(editor, v, p)
