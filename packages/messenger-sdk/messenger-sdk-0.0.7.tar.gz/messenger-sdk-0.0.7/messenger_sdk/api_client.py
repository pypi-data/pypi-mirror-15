...
class ApiClient:
    ...
    @gen.coroutine
    def async_request(self, url, http_method='GET', body=None, headers=None):
        url = self._validate_url(url)
        default_header = {'Content-Type': 'application/json'}
        headers = self.append_headers(default_header, headers)
...
class OAuthApiClient(ApiClient):
    ...
    @gen.coroutine
    def oauth_async_request(self, url, http_method='GET', body=None, headers=None):
        access_token = yield self._get_access_token()
        auth_header = {'Authorization': 'Bearer {access_token}'.format(access_token=access_token)}
        headers = self.append_headers(auth_header, headers)
        response = yield self.async_request(url=url, http_method=http_method, body=body, headers=headers)
        return response
