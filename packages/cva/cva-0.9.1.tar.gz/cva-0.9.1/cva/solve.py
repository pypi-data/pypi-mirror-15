...
def minpoint(sa, sb):
    ...
    # step 1: form a non-orthogonal basis space including our tangent vector
    basis = np.zeros((nparm,nparm))
    cartesian_space = np.zeros((nparm,nparm))
    for i in range(nparm):
        cartesian_space[i,i] = 1.0
    # our non-orthogonal basis must include the primary (tangent) unit vector
    ...
    # step 2: form an orthonormal basis including the tangent vector
    perp = np.empty((nparm,nparm))
    perp[0] = basis[0]        # perp[0] is the tangent unit vector
...
def straddle(s,step):
    ...
    try:
        ...
        N = len(s)-1
    incr = int(N/(2**step))
    for iter in range(_parms['tp_straddles']):
...
def run(sa,sb,steps=5,silent=False):
    ...
    nparm = len(sa)
    ndim = len(sa)+1        # in this release we support the case of R^{N-1} surfaces in R^N space
