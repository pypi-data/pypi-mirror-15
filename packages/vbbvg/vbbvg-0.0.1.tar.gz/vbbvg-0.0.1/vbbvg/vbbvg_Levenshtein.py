...
def ldist(str1, str2):
    "Return Levonshtein distance, corrected by strings lenghts."

    # ldist(u'Bäckerbrücke', u'Möckernbrücke (Berlin)') -> 2
    l1 = len(str1)
    l2 = len(str2)
    d = distance(str1, str2) - max(l1, l2) + min(l1, l2)
    return d


def find_similar_stops(df, stop, num_items=10):
    "Find a list of stop names most similar to the the one provided."

    # calculate Levinshtein distance for all stops in df to the given stop
    # and return dataframe with a certain number of those with the smallest
    # distance.

    df.insert(0, "Levinshtein", df.stop_name.apply(lambda dest: ldist(dest, stop)))
    df = df.sort(columns=['Levinshtein'])
    df = df[df.Levinshtein < 3]
...
def get_name_id_interactive(stop, df):
    ...
    if df1_len == 1:
        ...
        return result
    #### attemt to implement suggestions based on Levinshtein distance
    if 0: # df1_len == 0:
        msg = '"%s" not found. Did you mean one of the following stops, maybe?' % stop
        termcolor.cprint(msg, attrs=['bold'])
        df1 = find_similar_stops(df, stop)
        df1_len = len(df1)
        df1.drop('Levinshtein', axis=1, inplace=True)
        # return
    #### 
    if df1_len > 1:
