...
class ModelIntegration(models.Model):
    ...
    def active(self):
        ...
        if not AccountIntegration.objects.filter(
            ...
            print "RETORNO DO ACCOUNT INTEGRATION {}".format(result)
            result = False
        ...
        print "RETORNO DO ACTIVE {}".format(result)
        return result

    def get_object(self):
        selected_model = dict(
            MODEL_LIST).get(self.model_selected)
        default_manager = selected_model._default_manager
...
class IntegrationHistory(models.Model):
    ...
    account = models.ForeignKey(
        'plataforma.Conta',
        related_name='account_integration_history',
        on_delete=models.CASCADE)
    model_selected = models.CharField(
