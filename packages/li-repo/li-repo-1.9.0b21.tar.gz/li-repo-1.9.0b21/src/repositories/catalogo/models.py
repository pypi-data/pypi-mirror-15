...
from repositories.integration import serializers
from repositories.integration.models import ProdutoIntegracao, Integracao, ContaIntegracao
...
def send_notification(sender, instance, created, *args, **kwargs):
    ...
    # Para testes, se o produto nao tem na tabela de Integracao
    # Adicionar automaticamente
    if os.environ.get('ENVIRONMENT') == 'local':
        integracoes_conta = ContaIntegracao.objects.all()
        for integracao in integracoes_conta:
            if not ProdutoIntegracao.objects.filter(
                conta=instance.conta,
                integracao=integracao.integracao,
                produto=instance,
            ).exists():
                ProdutoIntegracao.objects.create(
                    conta=instance.conta,
                    integracao=integracao.integracao,
                    produto=instance,
                    data_inicio=timezone.now()
                )
                print "adicionado produto {} para {}".format(
                    instance.sku,
    ...
    lista_integracoes = [
        produtointegracao
        for produtointegracao in ProdutoIntegracao.objects.filter(
            conta=instance.conta,
            produto=instance
        )
        if produtointegracao.ativo()
    ]
    print "integracoes: %s" % (lista_integracoes)

    if lista_integracoes:
        json_dict = serializers.ProdutoSerializer(instance).serialize()
        if created:  # Create
                json_dict['connector_list'] = [
                    produtointegracao.integracao.slug
                    for produtointegracao in lista_integracoes
                ]
                ApiIntegration().send_product(
                    crud='create',
                    account_id=instance.conta.id,
                    product_sku=instance.sku,
                    product_dict=json.dumps(json_dict)
                )
        else:  # Update or Delete (Removed)
            for produtointegracao in lista_integracoes:
                if produtointegracao.removido == instance.removido:
                    json_dict['connector_list'] = list(
                        produtointegracao.integracao.slug)
                    ApiIntegration().send_product(
                        crud='update',
                        account_id=instance.conta.id,
                        product_sku=instance.sku,
                        product_dict=json_dict
                    )
                else:
                    crud = 'delete' if instance.removido else 'create'
                    produtointegracao.removido = instance.removido
                    produtointegracao.save()
                    json_dict['connector_list'] = list(
                        produtointegracao.integracao.slug)
                    ApiIntegration().send_product(
                        crud=crud,
...
def post_delete_produto(self, instance, *args, **kwargs):
    lista_integracoes = [
        integracao
        for integracao in ProdutoIntegracao.objects.filter(
            conta=instance.conta,
            produto=instance
        )
        if integracao.ativo()
