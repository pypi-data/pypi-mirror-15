    ...
    if lista_integracoes:
        ...
        if created:  # Create
                json_dict['connector_list'] = [
                    integracao.slug for integracao in lista_integracoes
                ]
            ...
            for integracao in lista_integracoes:
                if integracao.removido == instance.removido:
                    json_dict['connector_list'] = list(integracao.slug)
                    ApiIntegration().send_product(
                        crud='update',
                        account_id=instance.conta.id,
                        product_sku=instance.sku,
                    ...
                    integracao.removido = instance.removido
                    integracao.save()
                    json_dict['connector_list'] = list(integracao.slug)
                    ApiIntegration().send_product(
                        crud=crud,
...
def post_delete_produto(self, instance, *args, **kwargs):
    lista_integracoes = ProdutoIntegracao.objects.filter(
        conta=instance.conta,
