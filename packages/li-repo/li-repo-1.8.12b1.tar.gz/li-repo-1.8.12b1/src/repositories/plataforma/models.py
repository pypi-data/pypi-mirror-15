...
class GrupoNaoEncontrado(Exception):
    pass
...
class Usuario(models.Model):
    ...
    grupo = models.ForeignKey('plataforma.GrupoPermissao', related_name="usuarios", null=True)
    ...
    def has_privilege(self, scope_name):
        """ Verifica se o usuário pertence a algum grupo que tenha permissão
        de acessar o escopo solicitado
        """
        if not self.grupo:
            return False
        is_granted = Permissao.objects.filter(
            permitido=True,
            escopo__nome=scope_name,
            grupo__id=self.grupo.id)
        return True if is_granted else False
...
class Conta(models.Model):
    ...
    def assinar(cls, request, email, nome, senha, plano=None, contrato_id=None,
        ...
        grupos = GrupoPermissao.objects.filter(
            Q(padrao=True),
            Q(contrato_id=contrato_id) | Q(contrato_id__isnull=True))[:2]
        if not grupos:
            raise GrupoNaoEncontrado('Não foi possível encontrar nenhum grupo padrão. Contate o administrador.')
        grupo_padrao = None
        for grupo in grupos:
            # Prioridade em grupos de contrato
            if grupo.contrato_id and grupo.padrao:
                grupo_padrao = grupo
                break
        # Caso não ache nenhum grupo de contrato, retorne um grupo padrão global
        grupo_padrao = grupo_padrao or grupos[0]

        try:
            usuario = Usuario(
                email=email, nome=nome, senha=senha,
                contrato_id=contrato_id, grupo=grupo_padrao)
            usuario.save()
...
class GrupoPermissao(models.Model):
    """
    Um grupo de permissão define a lógica de acesso as áreas do painel.
    Ao associar permissões e escopos ao grupo é possível saber os direitos
    de acesso desse grupo. O mesmo deve conter um (1) ou mais
    usuários (Many-To-Many) do mesmo contrato apenas (Many-To-One).
    """
    id = custom_models.BigAutoField(db_column="id", primary_key=True)
    grupo_codigo = models.CharField(u'Nome identificador do grupo', db_column="grupo_codigo", max_length=100, unique=True)
    nome_exibicao = models.CharField(u'Nome de exibição do grupo', db_column="nome_exibicao", max_length=100)
    descricao = models.CharField(u'Descrição do grupo', db_column="descricao_grupo", max_length=255, null=True)
    padrao = models.BooleanField(u'Se o grupo é padrão para criação de novos usuários', db_column="padrao", default=False)
    contrato = models.ForeignKey('plataforma.Contrato', related_name="grupo_permissao", null=True)

    class Meta:
        db_table = u"plataforma\".\"tb_grupo_permissao"
        verbose_name = u"Grupo de permissões"
        verbose_name_plural = u"Grupo de permissões"

    def __unicode__(self):
        return self.grupo_codigo


class EscopoPermissao(models.Model):
    """
    Refere-se ao acesso delegado de uma área específica no painel.
    Para cada escopo existe uma implementação lógica para
    habilitar o acesso delegado a uma área específica.
    """
    id = models.AutoField(db_column="id", primary_key=True)
    nome = models.CharField(u'Nome do escopo', db_column="nome_escopo", unique=True, max_length=100)
    grupo = models.ManyToManyField('plataforma.GrupoPermissao', related_name='escopo_permissao')

    class Meta:
        db_table = u"plataforma\".\"tb_escopo_permissao"
        verbose_name = u"Escopo de permissões"
...
class Permissao(models.Model):
    """
    Representa o tipo de acesso que um determinado grupo associado a um escopo
    pode possuir. Uma permissão só tem funcionalidade caso esteja associada
    a um grupo e um escopo, dessa forma, um grupo consegue saber exatamente
    quais permissões estão relacionadas ao escopo configurado para o respectivo
    grupo.
    """
    id = custom_models.BigAutoField(db_column="id", primary_key=True)
    permitido = models.BooleanField(db_column="permitido", default=False)
    grupo = models.ForeignKey('plataforma.GrupoPermissao', related_name='permissao', on_delete=models.CASCADE)
    escopo = models.ForeignKey('plataforma.EscopoPermissao', related_name='permissao')

    class Meta:
        db_table = u"plataforma\".\"tb_permissao"
        verbose_name = u"Permissões"
        unique_together = ['escopo', 'grupo']

    def __unicode__(self):
        return '%s - %s' % (self.id, self.permitido)
...
class Feature(models.Model):
    ...
    @classmethod
    def is_enabled(cls, feature, store):
