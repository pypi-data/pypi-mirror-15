...
def main():
    ...
    if tool_name == 'flagstat':
        ...
        samtools_flagstat.samtools_flagstat(uuid, bam_path, reference_fasta_path, engine, logger)
    elif tool_name == 'idxstats':
        bam_path = pipe_util.get_param(args, 'bam_path')[0]
        reference_fasta_path = pipe_util.get_param(args, 'reference_fasta_path')
        samtools_idxstats.samtools_idxstats(uuid, bam_path, reference_fasta_path, engine, logger)
    elif tool_name == 'stats':
        bam_path = pipe_util.get_param(args, 'bam_path')[0]
        reference_fasta_path = pipe_util.get_param(args, 'reference_fasta_path')
        samtools_stats.samtools_stats(uuid, bam_path, reference_fasta_path, engine, logger)
    else:
