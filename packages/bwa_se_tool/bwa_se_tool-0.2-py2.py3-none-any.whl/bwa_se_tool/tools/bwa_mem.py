...
def bwa_mem(uuid, fastq_path, reference_fasta_path, readgroup_json_path, thread_count, engine, logger):
    step_dir = os.getcwd()
    fastq_name = os.path.basename(fastq_path)

    if fastq1_name.endswith('_s.fq.gz'):
        fastqbase_name = fastq1_name.replace('_s.fq.gz', '')
    elif fastq1_name.endswith('_s.fq'):
        fastqbase_name = fastq1_name.replace('_s.fq', '')
    elif fastq1_name.endswith('_o1.fq'):
        fastqbase_name = fastq1_name.replace('_o1.fq.gz', '')
    elif fastq1_name.endswith('_o1.fq'):
        fastqbase_name = fastq1_name.replace('_o1.fq', '')
    elif fastq1_name.endswith('_o2.fq'):
        fastqbase_name = fastq1_name.replace('_o2.fq.gz', '')
    elif fastq1_name.endswith('_o2.fq'):
        fastqbase_name = fastq1_name.replace('_o2.fq', '')
    else:
    ...
    if rg_str is None:
        bwa_cmd = [bwa_path, 'mem', '-t ' + str(thread_count), '-T', '0', reference_fasta_path,
                   + fastq_path]
    else:
