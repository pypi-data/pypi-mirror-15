...
class HTMLFormatter(base_formatter.Formatter):
    ...
    def __metric_classes(report):
        ...
        for metric_class in report.metric_classes():
            ...
            color = 'lightgrey' if metric_class in report.included_metric_classes() else 'black'
            try:
                ...
                raise
            result.append('<tr color="{color}"><td>{name}</td><td>{cls}</td><td>{qattr}</td><td>{norm}</td></tr>'.format(
                color=color, name=name, cls=class_name, qattr=quality_attribute, norm=norm))
        result.append('</table>')
        return '\n'.join(result)
