...
from qualitylib import utils, metric
...
class HTMLFormatterTest(unittest.TestCase):
    ...
    def test_included_metric_classes(self):
        """ Test that the report shows which metrc classes are included in the report and which are not. """
        report = fake_report.Report()
        html = self.__formatter.process(report)
        table_row = '<tr color="black"><td>{name}</td>'.format(name=report.metric_classes()[0].name)
    ...
    def test_not_included_metric_classes(self):
        ...
        report = fake_report.Report()
        html = self.__formatter.process(report)
        table_row = '<tr color="lightgrey"><td>{name}</td>'.format(name=metric.CyclomaticComplexity.name)
        self.assertTrue(table_row in html)
