		...
		for row in queryset:
			...
			for col in row:
				if isinstance(row[col], datetime.datetime):
					dct[col] = row[col].strftime(self.datetime_format)
				elif isinstance(row[col], datetime.date):
					dct[col] = row[col].strftime(self.date_format)
				else:
					dct[col] = row[col]
				# end if
...
class SupraFormView(FormView):
	...
	def form_valid(self, form):
		...
		return HttpResponse(serializers.serialize('json', [instance]), status=200, content_type="application/json")
	#end def
