...
class SupraListView(ListView):
	...
	def get_object_list(self, object_list):
		...
		if self.list_display:
			...
			queryset = queryset.values(*list_display)
		else:
		...
		def extra(dct, obj):
			...
			return dct
		# end def
		return self.format_json(queryset, extra)
	#end def

	@classmethod
	def format_json(cls, rows, extra=None, time=0):
		print time
		object_list = []
...
class SupraFormView(FormView):
	...
	def form_valid(self, form):
		...
		for inline in self.validated_inilines:
			...
			inline.save()
		#end for
		json_dict = SupraListView.format_json([instance.__dict__])
		return HttpResponse(json_dict, status=200, content_type="application/json")
	#end def
