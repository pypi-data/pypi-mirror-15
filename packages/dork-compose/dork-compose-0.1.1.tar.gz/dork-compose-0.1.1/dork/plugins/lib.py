...
class Plugin(dork.plugin.Plugin):
    ...
    @property
    def library(self):
        return os.path.expanduser(self.env.get('DORK_LIBRARY', None))
    ...
    def is_delocated(self, build):
        return (
            ...
            'dockerfile' in build
            and
            not os.path.exists('%s/%s' % (build['context'], build['dockerfile']))
            and
            os.path.exists('%s/%s' % (self.library, build['dockerfile']))
        )
