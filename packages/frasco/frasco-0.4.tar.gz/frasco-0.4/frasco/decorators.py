...
def preprend_base_url_to_expose(base_url, func):
    ...
    for i in xrange(len(func.urls)):
        rule, options = func.urls[i]
        rule = base_url.rstrip('/') + '/' + rule.lstrip('/')
        if rule.endswith('/'):
            rule = rule.rstrip('/')
        func.urls[i] = (rule, options)
...
def _make_marshaller_func(marshaller, **marshaller_kwargs):
...
def marshall_with(marshaller, **kwargs):
    def decorator(func):
        func.marshall_with = _make_marshaller_func(marshaller, **kwargs)
        return func
    return decorator


def marshall_many_with(marshaller, **kwargs):
    marshaller = _make_marshaller_func(marshaller, **kwargs)
    def many_marshaller(data, *args, **kwargs):
        return [marshaller(i, *args, **kwargs) for i in data]
    def decorator(func):
        func.marshall_with = many_marshaller
        return func
    return decorator