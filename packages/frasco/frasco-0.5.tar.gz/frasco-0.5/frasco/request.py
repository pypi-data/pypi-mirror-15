...
def process_request_params(params, target_kwargs):
    if not has_request_context():
        return {}
    for param in params:
        for name in param.names:
            target_kwargs.pop(name, None)
        target_kwargs.update(param.process())
    ...
    def decorator(func):
        if not hasattr(func, 'request_params'):
            ...
            def wrapper(*args, **kwargs):
                if auto_inject_request_params.top:
                    auto_inject_request_params.consume_once()
                    process_request_params(wrapper.request_params, kwargs)
