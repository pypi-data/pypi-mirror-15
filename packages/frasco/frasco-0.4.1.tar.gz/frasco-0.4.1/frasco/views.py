...
def full_exec_request_actions(actions, func=None, marshaller=None, render_func=None):
...
class ActionsView(View):
    ...
    def __init__(self, name=None, actions=None, func=None, template=None, marshaller=None,
                 auto_render=None, self_var=None, **kwargs):
    ...
    def dispatch_request(self, *args, **kwargs):
        ...
        if self.request_params:
            kwargs = process_request_params(self.request_params)
            current_context.vars.update(kwargs)
    ...
    def execute_actions(self, *args, **kwargs):
        ...
        return full_exec_request_actions(self.actions, func=func, marshaller=self.marshall,
            render_func=self._auto_render_func)
    ...
    def marshall(self, rv):
        if self.marshaller:
            return self.marshaller(rv)
        return rv
...
def as_view(url=None, methods=None, view_class=ActionsView, name=None, url_rules=None, **kwargs):
    ...
    def decorator(f):
        ...
        if hasattr(f, "marshall_with"):
            kwargs['marshaller'] = f.marshall_with

        clsdict = {"name": name or f.__name__,
                   "actions": getattr(f, "actions", None),
                   "url_rules": url_rules or getattr(f, "urls", None),
                   "request_params": getattr(f, "request_params", None)}
