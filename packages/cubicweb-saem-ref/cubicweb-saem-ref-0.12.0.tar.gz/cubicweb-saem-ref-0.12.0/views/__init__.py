...
from cubicweb.utils import JSString, UStringIO
...
def add_relations_button(req, entity, msg, *rtype_role_targets,
                         **extraurlparams):
    """Return an HTML dropdown button to add relations with `entity` as object"""
    links = [relation_link(req, entity, rtype, role, target, **extraurlparams)
...
def relation_link(req, entity, rtype, role, target_etype=None,
                  **extraurlparams):
    ...
    return add_etype_link(req, etype, text=req._(etype).lower(),
                          klass='', **urlparams)
...
def main_navigation_menu():
    ...
    for order, (path, label) in enumerate([
        ...
        ('SEDAProfile', 'SEDAProfile_plural'),
