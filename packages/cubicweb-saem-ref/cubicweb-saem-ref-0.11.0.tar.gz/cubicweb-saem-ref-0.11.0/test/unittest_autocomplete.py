...
from cubes.saem_ref.views.autocompletesearch import words_matching_term
...
class AutocompleteSearchTC(CubicWebTC):
    ...
    def _add_some_content(self):
        with self.admin_access.client_cnx() as cnx:
            testutils.agent(cnx, u'Secret Agent')
    ...
    def test_search(self):
        ...
        self.assertEqual(words, [{'label': 'secret : Agent, Concept', 'value': 'secret'}])
        words = self._search(term='secret', all='agent secret')
        self.assertEqual(words, [{'label': 'secret : Agent', 'value': 'secret'}])

    def test_facetted_search(self):
        self._add_some_content()
        words = self._search(term='secret', all='secret', facetrql='Any X WHERE X is Agent')
        self.assertEqual(words, [{'label': 'secret : Agent', 'value': 'secret'}])
