...
class Rest(object):
    ...
    def request(self, method, path, **kwargs):
        if 'headers' not in kwargs: kwargs['headers'] = self._headers()
    ...
    def async_request_raw(self, method, url, **kwargs):
        if 'headers' not in kwargs:
            kwargs['headers'] = self._headers(content_type='application/octet-stream')
    ...
    def async_request_flush(self, concurrency):
        ...
        self._async_reqs = []
        return grequests.map(reqs, size=concurrency, exception_handler=self._exception_callback)
