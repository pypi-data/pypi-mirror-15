...
class Api():
    ...
    def template_by_name(self, name):
        ...
        '''
        templates = request(self.oc_address, '/api/template/get_list/', {'token': self.token}, self.debug)
        return [Template(self.oc_address, self.token, template, self.debug) for template in templates if template['name'] == name]
    ...
    def network_by_name(self, name):
        ...
        Get network by name
        '''
        networks = request(self.oc_address, '/api/vm/get_list/', {'token': self.token}, self.debug)
        return [Network(self.oc_address, self.token, network, self.debug) for network in networks if network['name'] == name]
