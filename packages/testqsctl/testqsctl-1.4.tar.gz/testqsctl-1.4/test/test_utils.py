...
bad_config_sample = '''
bad_sample: 'this is a invalid config'
'''
...
class TestUtils(unittest.TestCase):
    ...
    def setUp(self):
        ...
        conf_file = "data/config/conf_file"
        bad_conf_file = "data/config/bad_conf_file"

        if not os.path.exists(conf_file):
            with open("data/config/conf_file", 'w') as f:
        ...
        with open("data/large_file", 'w') as f:
            f.seek(8*1024*1024)
            f.write("just for testing")
        self.large_file = "data/large_file"
    ...
    def test_encode_to_utf8(self):
        if is_windows():
            s = '\xd6\xd0\xce\xc4' # GBK encoded
            self.assertEqual(encode_to_utf8(s), '\xe4\xb8\xad\xe6\x96\x87')

    def test_encode_to_gbk(self):
        s = '\xe4\xb8\xad\xe6\x96\x87' # utf8 encoded
        self.assertEqual(encode_to_gbk(s), '\xd6\xd0\xce\xc4')

    def test_uni_print(self):
        notice = "utf8\xe5\xb7\xb2\xe6\x94\xaf\xe6\x8c\x81"
        uni_print(notice)
    ...
    def test_File_Chunk(self):
        ...
        self.assertEqual(part2.__len__(), len("just for testing"))
        self.assertEqual(part2.read(5), "just ")
        self.assertEqual(part2.read(), "for testing")
        part0.close()
        part1.close()
        part2.close()
