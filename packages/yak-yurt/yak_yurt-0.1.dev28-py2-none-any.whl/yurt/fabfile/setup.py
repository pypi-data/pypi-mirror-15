...
def get_settings():
    SETTINGS = get_fab_settings()
    PROJECT_NAME = get_project_name_from_repo(SETTINGS.get('git_repo'))
    SETTINGS['project_name'] = PROJECT_NAME
    SETTINGS['git_repo_url'] = SETTINGS.get('git_repo')
...
def create_project():
    ...
    :return: Void
    """
    SETTINGS, PROJECT_NAME = get_settings()
    local("cp -rf {0} ./{1}".format(os.path.join(DJANGO_PROJECT_PATH, "*"), PROJECT_NAME))
    recursive_file_modify(os.path.abspath("./{0}".format(PROJECT_NAME)), SETTINGS)
...
def load_orchestration_and_requirements():
    ...
    local('cp -f {0} ./{1}'.format(os.path.join(FABFILE_PATH, 'requirements.txt'), PROJECT_NAME))
    recursive_file_modify('./{0}/orchestration'.format(PROJECT_NAME), SETTINGS)
...
def enable_git_repo():
    ...
    with lcd(PROJECT_NAME):
        with settings(warn_only=True):
            local('git init')
            local('git remote add origin {0}'.format(SETTINGS.get('git_repo_url')))
...
def move_vagrantfile_to_project_dir():
    ...
    """
    SETTINGS, PROJECT_NAME = get_settings()
    local('mv ./{0}/orchestration/Vagrantfile .'.format(os.path.join(ORCHESTRATION_PROJECT_PATH,
                                                                     "Vagrantfile"), PROJECT_NAME))
