...
class GatewayUtilitySuppliesElectricity(GatewayBase):

    def __init__(self, endpoint, http_management=None):
        call = "utility/supplies/electricity"
        super(GatewayUtilitySuppliesElectricity, self).__init__(endpoint, call, http_management=http_management)

    def get(self, token, id):
        return super(GatewayUtilitySuppliesElectricity, self).get(token, id)
    ...
    def fetch(self, token, parameters=None):
        ...
        data = super(GatewayUtilitySuppliesElectricity, self).fetch(token, parameters)
        while len(data) == parameters["limit"]:
            parameters["start"] += 1
            some_devices = super(GatewayUtilitySuppliesElectricity, self).fetch(token, parameters)
