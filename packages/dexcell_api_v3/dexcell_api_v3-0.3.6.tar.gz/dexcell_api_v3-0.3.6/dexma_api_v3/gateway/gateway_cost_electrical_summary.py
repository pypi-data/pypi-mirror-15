...
class GatewayCostElectricalSummary(GatewayBase):

    def __init__(self, endpoint, http_management=None):
        call = "cost/electrical/summary"
        super(GatewayCostElectricalSummary, self).__init__(endpoint, call, http_management=http_management)
    ...
    def _basic_fetch(self, token, parameters, list_consumption):
        ...
        new_consumption = super(GatewayCostElectricalSummary, self).fetch(token, params)
    ...
    def check_embed_parameters(self, embed):
        if type(embed) != list: raise Exception("Embed parameter must be a valid list")
        for val in embed:
            if val not in ["prices", "device"]: raise Exception("Embed parameter must be or prices or devices not {}".format(val))
    ...
    def fetch(self, token, parameters=None):
        ...
        if "embed" in parameters: self.check_embed_parameters(parameters["embed"])
