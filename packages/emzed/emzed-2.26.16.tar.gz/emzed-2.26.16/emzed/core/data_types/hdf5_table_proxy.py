...
class Hdf5TableProxy(ImmutableTable):
    ...
    @profile
    def _prefetch(self, col_name):
        # reader caches values, so we just have to fetch data here:
        started = time.time()
        print("start prefetch")
        self.reader.get_raw_col_values(col_name)
        self.reader.get_col_values(col_name)
        print("prefetch done", time.time() - started)
