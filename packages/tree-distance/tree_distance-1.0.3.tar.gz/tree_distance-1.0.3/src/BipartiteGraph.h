...
public :
    BipartiteGraph(std::vector<std::vector<bool>>& IncidenceMatrix, const std::vector<double>& Aweight, const std::vector<double>& Bweight);
    vector<vector<size_t>> vertex_cover(const vector<size_t>& Aindex, const vector<size_t>& Bindex);
    static std::vector<std::vector<bool>> getIncidenceMatrix(std::vector<PhyloTreeEdge>& edges1, std::vector<PhyloTreeEdge>& edges2);
