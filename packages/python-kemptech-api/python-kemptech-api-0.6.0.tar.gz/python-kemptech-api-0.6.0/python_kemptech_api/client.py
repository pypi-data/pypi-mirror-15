...
class HttpClient(object):
    ...
    def __init__(self, tls_version=utils.DEFAULT_TLS_VERSION):
        ...
        self._tls_session.mount("http://", UseTlsAdapter(tls_version))
    ...
    def _do_request(self, http_method, rest_command,
                    parameters=None,
                    file=None, headers=None, cert=None):
    ...
    def _get_curl_command_list(self, command):
        return ['curl', '-s', '-k', '--connect-timeout', str(utils.TIMEOUT),
...
class LoadMaster(HttpClient):
    ...
    def __init__(self, ip, username, password, port=443, cert=None,
    ...
    def build_virtual_service(self, service, response=None):
        ...
        if is_sub_vs:
            ...
            data = get_data(response)
            subvs_data = get_sub_vs_list_from_data(data)[1]
    ...
    def add_local_user(self, user, password, radius=False):
        params = {
            'user': user,
            'password': password,
    ...
    def download_user_cert(self, user):
        file_name = "{}.cert".format(user)
