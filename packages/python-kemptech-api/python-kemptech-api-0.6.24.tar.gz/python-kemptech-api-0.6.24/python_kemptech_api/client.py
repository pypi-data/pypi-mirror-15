...
class LoadMaster(HttpClient):
    ...
    def get_all_objects(self):
        ...
        for vs in virtual_services:
            vs.subvs_entries = []
            print(vs.subvs_entries)
            parent_vs_id = vs.mastervsid
            if parent_vs_id != 0:
                for parent_vs in virtual_services:
                    if parent_vs.index == parent_vs_id:
                        parent_vs.subvs_entries.append(vs)
        """
        for vs in virtual_services:
...
class KempBaseObjectModel(object):
    ...
    _API_IGNORE = (
        ...
        "subvs_data", "subvs_entries", "real_servers", "cert", "rsminimum",
