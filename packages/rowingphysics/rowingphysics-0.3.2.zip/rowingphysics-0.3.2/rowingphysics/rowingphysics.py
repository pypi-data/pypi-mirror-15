...
alfa = 3.16 # best fit to Kleshnev data for single 
...
def empirical(datafile,vavg,crew,rigging,tstroke,trecovery,doplot=1):
    ...
    Fdrag = drag_eq((Nrowers*mc)+mb,xdot,doprint=0,alfa_ref=alfa)
    ...
    Pw = drag_eq((Nrowers*mc)+mb,xdot,alfaref=alfa)*xdot
...
def energybalance(F,crew,rigging,v0=4.3801,dt=0.03,doplot=1,doprint=0,
  ...
  # stroke parameters
   ...
   while (vcstroke < vcstroke2):
       ...
       ydot[i] = vcstroke
       Fdrag = drag_eq((Nrowers*mc)+mb,xdot[i-1],alfaref=alfa)
...
#      vcstroke = vhand*(1-(handlepos/d))
   ...
   if (recprofile == 1): # oude methode (sinus)
      ...
      for k in range(i+1,aantal):
         Fdrag = drag_eq((Nrowers*mc)+mb,xdot[k-1],alfaref=alfa)
...
#   Ekinc = 0.5*mc*(xdot+ydot)**2 - 0.5*mc*v0**2
   ...
   Pw = drag_eq((Nrowers*mc)+mb,xdot,alfaref=alfa)*xdot
   Pwmin = drag_eq((Nrowers*mc)+mb,mean(xdot),alfaref=alfa)*mean(xdot)
...
def drag_skif():
    ...
    W1 = drag_eq(displacement,velo,alfaref=alfa)
    W2 = a2*velo**2
