...
class Service(metaclass=ServiceMetaClass):
    ...
    def _listen_to_port(self, host, port=8000, n=0):
        try:
            # assign the port to the app instance
            self.app.listen(port, address=host)
        except:
            # only go through a 1000 ports
            if n < 1000:
                # try the next port
                return self._listen_to_port(host, port=port+1, n=n+1)
            # otherwise the service has tried 1000 times, its tired.
            else:
                # yell loudly
