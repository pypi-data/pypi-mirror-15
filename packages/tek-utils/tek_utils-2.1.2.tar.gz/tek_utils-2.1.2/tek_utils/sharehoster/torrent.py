...
SearchResult = collections.namedtuple(
    'SearchResult', ('title', 'size', 'seeders', 'magnet_link')
)
...
class SearchResultFactory(object):
    ...
    def from_tpb(self, result):
        return SearchResult(result.title, result.size, result.seeders,
                            result.magnet_link)

    @classmethod
    def from_kickass(self, result):
        return SearchResult(result.name, result.size, result.seed,
                            result.magnet_link)
...
class SearchResultChoice(Input):
    ...
    def _init_strings(self):
        ...
        size_strings = [f.size for f in self.files]
