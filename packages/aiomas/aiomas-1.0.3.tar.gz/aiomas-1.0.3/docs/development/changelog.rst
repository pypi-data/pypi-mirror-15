...
- [FIX] The function :func:`asyncio.ensure_future()` called in
  :func:`aiomas.util.create_task()` was introduced in Python 3.4.4 and is not
...
- [CHANGE] :func:`aiomas.util.create_task()` replaces
  :func:`aiomas.util.async()`.  :func:`aiomas.util.async()` is now deprecated
  and will be removed in aiomas 2 and when a new Python release no longer
  allows to use *async* as name.
...
- [NEW] A Contract Net example
  (https://bitbucket.org/sscherfke/aiomas/src/tip/examples/agent_contractnet.py)
