...
def get_influencers(username, client, num_results = 10):
    user = client.nodes("username = '" + username + "'")
    if len(user) != 1:
        return "Unknown User"
    ppr_map = client.ppr(user, max_results=num_results+1, num_steps = default_num_steps, reset_probability=default_reset_probability)
    influencer_ids = filter(lambda id: id != user[0],
        sorted(ppr_map, key = lambda id: -ppr_map[id]))
    id_to_username = client.multi_node_attribute(influencer_ids, "username")
    return [id_to_username[i] for i in influencer_ids]

def get_recommendations(username, client, num_results = 10):
    user = client.nodes("username = '" + username + "'")
    if len(user) != 1:
        return "Unknown User"
    user_id = user[0]
    ...
    ppr_map = client.ppr(user, max_results=num_results_to_fetch, num_steps = default_num_steps, reset_probability=default_reset_probability)
    ...
    id_to_username = client.multi_node_attribute(recommended_ids, "username")
...
if __name__ == "__main__":
    if len(sys.argv) == 2:
        ...
        print "TOP INFLUENCERS: ", get_influencers(sys.argv[1], client)
        print "TOP RECOMMENDATIONS: ", get_recommendations(sys.argv[1], client)
        client.close()
    else:
