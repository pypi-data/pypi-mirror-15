...
:statuscode 401: unauthorized request
        ...
        sort_by = self.get_argument('sort_by', 'started')
        reverse = self.get_argument('reverse', default=True, type=bool)
        if reverse:
            sort_by = "-%s" % sort_by
        ...
        filted_tasks = tasks.iter_tasks(
            ...
            convert_datetime_format="%Y-%m-%d %H:%M:%S"
        )

        for _, task in filted_tasks:
            task = tasks.as_dict(task)
            if not task.get("name"):
                continue

            _worker = task.pop("worker", None)
            if _worker and hasattr(_worker, 'hostname'):
                task["workername"] = _worker.hostname
            elif isinstance(_worker, dict):
                task["workername"] = _worker.get("hostname", "-")
            else:
                task["workername"] = '-'

            result.append(task)

        # here, chagne to dict, make sort failed
        #  self.write(dict(result))
        total = len(result)
        self.write(dict(data=result, total=total))
