...
def iter_tasks(events, limit=None, type=None, task_id=None, worker=None, state=None,
               sort_by=None, received_start=None, received_end=None,
               started_start=None, started_end=None, search_terms=None, convert_datetime_format='%Y-%m-%d %H:%M'):
    i = 0
    ...
    convert = lambda x: time.mktime(
        datetime.datetime.strptime(x, convert_datetime_format).timetuple()
    )
    ...
    for uuid, task in tasks:
        if task_id and task_id not in uuid:
            continue
...
def sort_tasks(tasks, sort_by):
    ...
    if sort_by.startswith('-'):
        ...
        reverse = True
    # when sort_by == started, some task not start yet, we care more about it.
    if sort_by != 'started':
        key = lambda x: getattr(x[1], sort_by) or sort_keys[sort_by]()
    else:
        key = lambda x: getattr(x[1], sort_by) or getattr(x[1], 'received') or sort_keys[sort_by]()
    for task in sorted(
            tasks,
            key=key,
            reverse=reverse):
