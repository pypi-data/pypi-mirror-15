...
class LibraryFacade(object):
    ...
    def scan_episodes(self):
        ...
        '''
        logger.info('Rescanning episode collections…')
        files = itertools.chain(*[c.video_files for c in
                                  self._episode_collections])
        new_files = 0
        for _file in files:
            if is_episode(_file):
                data = self._epimeta_factory.from_filename(_file)
                new = not self._episode_exists(data)
                if new:
                    new_files += 1
                self._add_new_episode(data, new=new)
        logger.info('Done. Found {} new episodes.'.format(new_files))

    def scan_movies(self):
        logger.info('Rescanning movie collections…')
        files = itertools.chain(*[c.video_files for c in
        ...
        logger.info('Done. Found {} new movies.'.format(new_files))
    ...
    def clean_episodes(self):
        ...
        for episode in self.episodes():
            if not any([c.contains_video(episode) for c in
                ...
                removed += 1
        logger.info('Done. Removed {} episodes.'.format(removed))
    ...
    def clean_movies(self):
        ...
        for movie in self.movies():
            if not any([c.contains_video(movie) for c in
                ...
                movie.removed = True
                removed += 1
        logger.info('Done. Removed {} movies.'.format(removed))
