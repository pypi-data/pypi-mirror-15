...
class FeedPoller(ReleaseHandler):
    ...
    def __init__(self, releases, *a, **kw):
        super(FeedPoller, self).__init__(releases, self._rss_interval,
                                         'feed poller')
        self._rss = None
    ...
    def _reconstruct_entries(self):
        if self._rss and self._rss.entries:
            ...
            logger.info(text.format(len(self._feed_entries), fact.domain))
    ...
    def _create_monitor(self, entry):
        ...
        if old_entry is not None:
            if not entry.is_fix:
                monitor.downloaded = old_entry.downloaded
                monitor.archived = old_entry.archived
            old_entry.nuke(monitor)
        self._releases.add(monitor)
        logger.info('Added monitor for {!s}'.format(monitor.release))
