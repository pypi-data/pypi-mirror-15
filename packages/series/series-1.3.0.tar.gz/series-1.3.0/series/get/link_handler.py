...
class LinkCheckTask(Process):
    ...
    def __init__(self, link):
        ...
        super(LinkCheckTask, self).__init__(name='seriesd link checker')
        self.get = True

    def run(self):
        kit.l.debug('Checking link {}…'.format(self.link.url))
        self._check_link()
        self.done.value = 1
...
class LinkHandler(ReleaseHandler):
    ...
    def __init__(self, releases, *a, **kw):
        self._running_tasks = []
        super(LinkHandler, self).__init__(releases, 1, 'link handler', **kw)
