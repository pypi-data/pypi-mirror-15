...
class LinkChecker(object):
    ...
    def downloader(self):
        try:
            ...
            self.status = HttpLink.CHECK_FAILED
            self.reason = str(e)
            logger.debug('LinkChecker: {}'.format(e))
...
class HttpLink(Link):
    ...
    def __init__(self, url):
        super(HttpLink, self).__init__(url=url)
...
class TorrentLink(Link):
    ...
    def __init__(self, url):
        super(TorrentLink, self).__init__(url=url)
        self.cached = False
    ...
    @lazy_property
    def torrent(self):
        return TorrentProxy(self.url)

    @property
    def valid(self):
        return (self.url is not None and self.cached and self.download_url is
                not None)
    ...
    def __str__(self):
        s = '<TorrentLink valid: {} url: {}>'
        return s.format(self.valid, self.url)
    ...
    @property
    def cachable(self):
        return not (self.torrent.caching or self.valid)
