...
class TorrentFinder(ReleaseHandler):
    ...
    def __init__(self, releases, *a, **kw):
        super(TorrentFinder, self).__init__(releases, 5, 'torrent finder',
                                            **kw)
    ...
    def _process_results(self, monitor, results, search_string):
        ...
        if matches:
            link = matches[0].magnet_link
            if link:
                logger.info('Added torrent to release "{}"'.format(release))
                monitor.add_link(link)
                return True
            else:
                logger.error('Could not extract magnet link from kickass' +
                             ' search results: {}'.format(matches[0]))
        else:
    ...
    def _qualify(self, monitor):
        return (not monitor.downloaded and not monitor.torrent and
