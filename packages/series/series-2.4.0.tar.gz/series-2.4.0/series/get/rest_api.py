...
class RestApi(RestApiBase, Tvdb):
    ...
    @route('/release/explain')
    @route('/release/explain/<int:num>')
    def explain(self, num=5):
    ...
    def release_list(self):
        regex = request.get_json().get('regex', '')
    ...
    def update_release(self, _id):
        if self._releases.find_by_id(_id):
            return self._releases.update_by_id(_id, **request.get_json()).info
        else:
            return 'Release doesn\'t exist'
    ...
    def list_shows(self):
        regex = request.get_json().get('regex', '')
        matches = self._shows.filter_by_regex(regex)
        return [[str(s.id), str(s.name)] for s in matches]
