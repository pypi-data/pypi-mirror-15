...
def spade2d(image, shapes_library, threshold, data_binding_function=None,
    ...
    Main SPADE function.
    """
    # If no mask or poi is passed, creates bool arrays filled with True,
    # same dimensions as img, i.e. inspect all pixels.
    if mask is None:
    ...
    for centered_y, centered_x in np.argwhere(extended_potential_centers):
        ...
        if possible_shapes_bool.any():
            ...
            ring_window = (slice(real_y - 1,
                                 real_y + shapes_library.grid_size + 1),
                           slice(real_x - 1,
                                 real_x + shapes_library.grid_size + 1)
                           )
