...
class Vertex(object):
	def __init__(self, canvas, position, num_sides):
		...
		n, r, r2 = self.num_sides, 3*self.num_sides, 3*self.num_sides+5
		self.r = r
		self.edges = [
			Edge(self.canvas, [self.position, (self.position[0] + r2*cos(i * 2 * pi / n), self.position[1] + r2*sin(i * 2 * pi / n))])
			for i in range(self.num_sides)
			]
		self.drawn = self.canvas.create_oval(
			[p + scale*r for scale in [-1, 1] for p in self.position],
			outline='black', fill='grey', tag='oval'
			)
...
class Edge(object):
	def __init__(self, canvas, positions):
		...
		self.head = self.canvas.create_oval(
			...
			outline='black', fill='red', tag='oval'
			)
