...
# $Id: madrigalWeb.py 5726 2016-06-17 14:19:18Z brideout $
...
class MadrigalData:
    ...
    def isprint(self, filename, parms, filters, user_fullname, user_email, user_affiliation, outputFile=None):
        ...
        if not outputFile is None:
            ...
            if format in ('Hdf5', 'netCDF4'):
                ...
                if distutils.version.LooseVersion(version) < distutils.version.LooseVersion('3.0.0'):
                    raise ValueError, 'Madrigal site at %s is below 3.0.0, cannot convert to Hdf5 or netCDF4' % (self.cgiurl)
        ...
        if page.find('Error occurred') != -1 and format == 'ascii':
    ...
    def downloadFile(self, filename, destination, user_fullname, user_email, user_affiliation, 
        ...
        if format == 'blockedBinary':
            ...
            # verify Madrigal site can handle this argument
            if self.compareVersions('3.0', self._madVers) > 0:
        ...
        if fileType > 0:
            if not self.compareVersions('3.0', self._madVers) > 0:
    ...
    def downloadWebFile(self, expPath, destination):
        ...
        url = os.path.join(self.cgiurl,'downloadWebFileService.py')
