...
def Run(encoding=None, cmdline_values_as_bytes=False):
    "parses command-line and compares with either func or, if None, Script.command"
    global SYS_ARGS, ENCODING, CMDLINE_VALUES_AS_BYTES
    debug('Run entered')
    ...
    ENCODING = encoding
    CMDLINE_VALUES_AS_BYTES = cmdline_values_as_bytes
    if py_ver < (3, 0):
        SYS_ARGS = [arg.decode(encoding or LOCALE_ENCODING) for arg in sys.argv]
    else:
        SYS_ARGS = sys.argv[:]
    try:
