...
def prepare_test_env(branch):
    ...
    try:
        if not exec_command_with_retry("git clone -b {0} --single-branch git@github.com:{1}/{2}.git {1}"
                                       .format(branch,
...
def get_config(package_name, test_env_path, full=False):
    ...
    if not full:
        for i in config['packages']:
                if i['pkg_name'] == package_name:
                    pkg_config = i
                    break
    else:
        pkg_config = config

    logger.info('Conf is ok and has keys {0}'.format(config.keys()))
    return pkg_config
