...
class profile_details(object):
    ...
    def readFromFile(cls, path):
        ...
        if data.find(profile_details.__PLIST_START) == -1:
            print("## Not a real provisioning profile {} ##".format(path))
            return None
        xml = data[data.find(profile_details.__PLIST_START): data.find(
            profile_details.__PLIST_END) + len(profile_details.__PLIST_END)]
