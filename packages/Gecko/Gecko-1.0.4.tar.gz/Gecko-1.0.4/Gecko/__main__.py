...
def run(args, ):
    ...
    for sensorname, sensor in sensors.iteritems():
        ...
        if (issubclass(sensor, GeckoIO.Boards.ControlAnything.IOm)) and (gecko.ControlAnything == None):
            logger.error("Tried to add a Control Anything sensor when no ControlAnything board was detected")
            return
        elif (issubclass(sensor, GeckoIO.Boards.GrovePi)) and (gecko.GrovePi == None):
...
def init(args):
    ...
    sensors = sensor_template.substitute(sensorInfo)

    sensorsFileContents = sensors_template.substitute({'sensors': sensors})
