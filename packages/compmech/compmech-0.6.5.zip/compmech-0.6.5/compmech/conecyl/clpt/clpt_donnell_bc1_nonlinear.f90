subroutine(npts, xs, ts, out, alphas, args)
    implicit none
    integer, pointer :: npts
    double precision, pointer, intent(in) :: xs(:), ts(:), out(:), alphas(:)
    type cc_attributes 
        double precision, pointer :: sina
        double precision, pointer :: cosa
        double precision, pointer :: tLA
        double precision, pointer :: r2
        double precision, pointer :: L
        double precision, pointer :: F(:)
        integer, pointer :: m1
        integer, pointer :: m2
        integer, pointer :: n2
        double precision, pointer :: coeffs(:)
        double precision, pointer :: c0(:)
        integer, pointer :: m0
        integer, pointer :: n0
    end type cc_attributes
    integer :: i1, k1, i2, j2, k2, l2, c, i, pos
    double precision :: A11, A12, A16, A22, A26, A66
    double precision :: B11, B12, B16, B22, B26, B66

    double precision :: p00, p01, p02, p10, p11, p12
    double precision :: p20, p21, p22, p30, p31, p32
    double precision :: p40, p41, p42, p50, p51, p52
    double precision :: q02, q04, q05, q14, q15, q22, q24, q25

    double precision :: r, x, t, alpha

    double precision, target :: F(:)
    double precision, target :: coeffs(:)
    double precision, target :: c0(:)
    double precision, target :: sina, cosa, tLA, r2, L
    integer, target :: m0, n0, m1, m2, n2
    double precision :: wx, wt, w0x, w0t

    cc_attributes :: args_in
        ...
        for i1 in range(i0, m1+i0):
            ...
            for k2 in range(i0, m2+i0):
                for l2 in range(j0, n2+j0):
                    ...
                    # kG_12
                    c += 1
                    out[c] += alpha*(p20*q04 + p21*q14)
                    c += 1
                    out[c] += alpha*(p20*q05 + p21*q15)
            ...
            for j2 in range(j0, n2+j0):
                ...
                for k2 in range(i2, m2+i0):
                    for l2 in range(j2, n2+j0):
                        ...
                        # kG_22
                        c += 1
                        out[c] += alpha*(p40*q04 + p41*q14)
                        c += 1
                        out[c] += alpha*(p40*q05 + p41*q15)
                        c += 1
                        out[c] += alpha*(p50*q04 + p51*q14)
                        c += 1
                        out[c] += alpha*(p50*q05 + p51*q15)
...
def calc_fint(np.ndarray[cDOUBLE, ndim=1] coeffs,
              double alpharad, double r2, double L, double tLA,
              np.ndarray[cDOUBLE, ndim=2] F,
              int m1, int m2, int n2,
              int nx, int nt, int num_cores, str method,
    ...
    # numerical integration
    if method=='trapz2d':
        trapz2d(<f_type>cffint, fdim, fint, xa, xb, nx, ta, tb, nt,
                &args, num_cores)
    elif method=='simps2d':
        simps2d(<f_type>cffint, fdim, fint, xa, xb, nx, ta, tb, nt,
                &args, num_cores)
...
cdef void cffint(int npts, double *xs, double *ts, double *fint,
    ...
    for i in range(npts):
        ...
        for i1 in range(i0, m1+i0):
            ...
            gxtL += pi*c[col+2]*cosi1x*i1*(w0t + wt)/(L*r)
        ...
        for j2 in range(j0, n2+j0):
            ...
            for i2 in range(i0, m2+i0):
                ...
                ettL += (0.5*c[col+4]*sini2x*(cosj2t*j2*(2*w0t + wt))/(r*r)
                         + 0.5*c[col+5]*sini2x*(-j2*sinj2t*(2*w0t + wt))/(r*r))
                ...
                gxtL += (c[col+4]*(L*cosj2t*j2*sini2x*(w0x + wx) + pi*cosi2x*i2*sinj2t*(w0t + wt))/(L*r)
                        + c[col+5]*(-L*j2*sini2x*sinj2t*(w0x + wx) + pi*cosi2x*cosj2t*i2*(w0t + wt))/(L*r))
        ...
        fint[0] += alpha*((NttL*sina*(L - x) - NxxL*r)/(L*cosa))
        fint[1] += alpha*(-NxtL*r2*(r + sina*(L - x))/L)
        fint[2] += alpha*((L*NttL*sina - NttL*sina*x + NxtL*(L - x)*sin(t - tLA) - NxxL*r + (NttL*sina*(-L + x) + NxxL*r)*cos(t - tLA))/(L*cosa))

        for i1 in range(i0, m1+i0):
            col = (i1-i0)*num1 + num0
            sini1x = vsini1x[i1-i0]
            cosi1x = vcosi1x[i1-i0]
            fint[col+0] += alpha*(NttL*sina*sini1x + pi*NxxL*cosi1x*i1*r/L)
            fint[col+1] += alpha*(-NxtL*sina*sini1x + pi*NxtL*cosi1x*i1*r/L)
            fint[col+2] += alpha*((-pi*L*cosi1x*i1*(MttL*sina - Nxt0*(w0t + wt) - NxtL*w0t - NxtL*wt - Nxx0*r*w0x - Nxx0*r*wx - NxxL*r*w0x - NxxL*r*wx) + sini1x*((L*L)*NttL*cosa + (pi*pi)*MxxL*(i1*i1)*r))/(L*L))
