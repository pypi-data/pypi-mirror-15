...
cdef void cfuvw(double *c, int m1, int m2, int n2, double r2, double L,
    ...
    cdef double sinix, cosix, sinj2t, cosj2t, u, v, w, x, t
    ...
    for i in range(size):
        ...
        for j2 in range(j0, n2+j0):
            ...
            for i2 in range(i0, m2+i0):
                ...
                sinix = sin(i2*pi*x/L)
                cosix = cos(i2*pi*x/L)
                u += c[col+0]*cosix*sinj2t
                u += c[col+1]*cosix*cosj2t

                v += c[col+2]*sinix*sinj2t
                v += c[col+3]*sinix*cosj2t

                w += c[col+4]*sinix*sinj2t
                w += c[col+5]*sinix*cosj2t
...
cdef void cfv(double *c, int m1, int m2, int n2, double *xs, double *ts,
    ...
    cdef int i1, i2, j2, col, i
    cdef double sinix, sinj2t, cosj2t, x, t, v
...
cdef void cfwt(double *c, int m1, int m2, int n2, double *xs, double *ts,
    ...
    cdef double sinix, sinj2t, cosj2t, wt, x, t
    ...
    for i in range(size):
        ...
        for j2 in range(j0, n2+j0):
            ...
            for i2 in range(i0, m2+i0):
                col = (i2-i0)*num2 + (j2-j0)*num2*m2 + num0 + num1*m1
                sinix = sin(i2*pi*x/L)
                wt += sinix*(j2*cosj2t)*c[col+4]
                wt += sinix*(-j2*sinj2t)*c[col+5]
        outwt[i] = wt
...
cdef cfgss(double[:,::1] gss, int m1, int m2, int n2,
           double r2, double x, double t, double L, double cosa, double tLA):
    cdef double sinix, cosix, sinj2t, cosj2t
    ...
    for i2 in range(i0, m2+i0):
        ...
        for j2 in range(j0, n2+j0):
            ...
            sinj2t = sin(j2*t)
            cosj2t = cos(j2*t)
            gss[0, col+0] = cosix*sinj2t
            gss[0, col+1] = cosix*cosj2t

            gss[1, col+2] = sinix*sinj2t
            gss[1, col+3] = sinix*cosj2t

            gss[2, col+4] = sinix*sinj2t
            gss[2, col+5] = sinix*cosj2t
    ...
    for i in range(size):
        ...
        for j2 in range(j0, n2+j0):
            ...
            for i2 in range(i0, m2+i0):
                ...
                ett += (c[col+0]*cosi2x*sina*sinj2t/r
                        + c[col+1]*cosi2x*cosj2t*sina/r
                        + 0.5*c[col+2]*sini2x*(cosa*sinj2t*(cosa*v - 2*w0t - wt) + 2*cosj2t*j2*r)/(r*r)
                        + 0.5*c[col+3]*sini2x*(cosa*cosj2t*(cosa*v - 2*w0t - wt) - 2*j2*r*sinj2t)/(r*r)
                ...
                gxt += (c[col+0]*cosi2x*cosj2t*j2/r
                        -c[col+1]*cosi2x*j2*sinj2t/r
                        + 0.5*c[col+2]*sinj2t*(-sini2x*(cosa*(2*w0x + wx) + 2*sina)/r + 2*pi*cosi2x*i2/L)
                        + 0.5*c[col+3]*cosj2t*(-sini2x*(cosa*(2*w0x + wx) + 2*sina)/r + 2*pi*cosi2x*i2/L)
                ...
                ktt += (c[col+2]*cosa*cosj2t*j2*sini2x/(r*r)
                        -c[col+3]*cosa*j2*sini2x*sinj2t/(r*r)
                        + c[col+4]*sinj2t*(L*(j2*j2)*sini2x - pi*cosi2x*i2*r*sina)/(L*(r*r))
                        + c[col+5]*cosj2t*(L*(j2*j2)*sini2x - pi*cosi2x*i2*r*sina)/(L*(r*r)))

                kxt += (c[col+2]*cosa*sinj2t*(-L*sina*sini2x + pi*cosi2x*i2*r)/(L*(r*r))
                        + c[col+3]*cosa*cosj2t*(-L*sina*sini2x + pi*cosi2x*i2*r)/(L*(r*r))
