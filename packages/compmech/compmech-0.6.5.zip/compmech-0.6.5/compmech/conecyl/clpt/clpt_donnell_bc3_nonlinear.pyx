    ...
    for i in range(npts):
        ...
        for i2 in range(i0, m2+i0):
            ...
            for j2 in range(j0, n2+j0):
                ...
                p20 = -pi*A16*i2*r*sini2x*sinj2t/L + cosi2x*(A12*cosj2t*j2 - A16*sina*sinj2t)
                p21 = -pi*A26*i2*r*sini2x*sinj2t/L + cosi2x*(A22*cosj2t*j2 - A26*sina*sinj2t)
                p22 = -pi*A66*i2*r*sini2x*sinj2t/L + cosi2x*(A26*cosj2t*j2 - A66*sina*sinj2t)
                p30 = -pi*A16*cosj2t*i2*r*sini2x/L - cosi2x*(A12*j2*sinj2t + A16*cosj2t*sina)
                p31 = -pi*A26*cosj2t*i2*r*sini2x/L - cosi2x*(A22*j2*sinj2t + A26*cosj2t*sina)
                p32 = -pi*A66*cosj2t*i2*r*sini2x/L - cosi2x*(A26*j2*sinj2t + A66*cosj2t*sina)
...
cdef void cffint(int npts, double *xs, double *ts, double *fint,
    ...
    for i in range(npts):
        ...
        ettL = castro*(cosa*w0/r + 0.5*w0t*w0t/(r*r))
        ...
        for i1 in range(i0, m1+i0):
            ...
            gxtL += c[col+2]*(pi*cosi1x*i1*w0t/(L*r) + 0.5*pi*cosi1x*i1*wt/(L*r))
        ...
        for j2 in range(j0, n2+j0):
            ...
            for i2 in range(i0, m2+i0):
                ...
                exxL += (c[col+5]*(pi*cosi2x*cosj2t*i2*w0x/L + 0.5*pi*cosi2x*cosj2t*i2*wx/L)
                         + c[col+4]*(pi*cosi2x*i2*sinj2t*w0x/L + 0.5*pi*cosi2x*i2*sinj2t*wx/L))
        ...
        for j2 in range(j0, n2+j0):
            ...
            for i2 in range(i0, m2+i0):
                ...
                fint[col+2] = beta*(fint[col+2]) + alpha*((L*NttL*cosi2x*cosj2t*j2 - NxtL*sinj2t*(L*cosi2x*sina + pi*i2*r*sini2x))/L)
                fint[col+3] = beta*(fint[col+3]) + alpha*(-(L*NttL*cosi2x*j2*sinj2t + NxtL*cosj2t*(L*cosi2x*sina + pi*i2*r*sini2x))/L)
