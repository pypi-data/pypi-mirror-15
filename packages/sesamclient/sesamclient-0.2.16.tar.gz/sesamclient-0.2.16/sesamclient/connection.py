...
class Connection:
    ...
    def upload_config(self, config,
                      replace_all_existing_config=True,
                      force=False,
                      _use_multipart_form_data=True):
        """
        Uploads the specified configuration.
        ...
        :param replace_all_existing_config: If this is True, the new configuration will replace any existing
                      configuration. Any existing configuration entities that are not in the new configuration will
                      be removed.
                      If is is False, no existing configuration entities will be deleted.
        ...
        if replace_all_existing_config:
            # we want to blat the existing configuration, so use a PUT-request
            request_method = self.do_put_request
        else:
            # we want to add to (or modify) the existing configuration, so use a POST-request
            request_method = self.do_post_request
        ...
        try:
            if _use_multipart_form_data:
                ...
                response = request_method(self.config_url,
                    ...
                    if not isinstance(data, bytes):
                        raise AssertionError(
                            "The content-type is 'application/zip', but the data isn't of type 'bytes': type(data):%s"
                                % (type(data),))
                response = request_method(self.config_url, data=data,
                                          params=request_params,
