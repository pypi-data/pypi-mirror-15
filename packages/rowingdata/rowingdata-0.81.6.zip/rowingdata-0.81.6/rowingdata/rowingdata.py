...
def phys_getpower(velo,rower,rigging,bearing,vwind,winddirection):
	...
	if (power>150):
	    try:
		reserg = rowingphysics.constantwatt_erg(power,rower,theerg,theconst=1.0)
	    except:
		reserg = [0,0,0,0,0]
	else:
	    reserg = [0,0,0,0,0]
...
class ErgDataParser:
	...
	# create unixtime using date
	dateobj = parser.parse(self.row_date)
	unixtime = seconds2+time.mktime(dateobj.timetuple())
...
class rowingdata:
	...
	df['cum_dist'] = make_cumvalues(df[' Horizontal (meters)'])
