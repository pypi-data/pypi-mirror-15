...
from webparser_user_info import get_user_info
...
class TwitterCrawler:
    ...
    def __init__(self, job):
        ...
        if 'info' in self._job:
            self._extension = '.info'
        elif 'timeline' in self._job:
            self._extension = '.timeline'
        elif 'threads' in self._job:
            self._extension = '.threads'
        elif 'tweet' in self._job:
    ...
    def log(self, message, is_close=False):
        if isinstance(self._notifier, Gmail):
            ...
            print self._notifier._create_at
        ...
        with open(self._output, 'a') as o:
            ...
            for item in self._input:
                ...
                if random() > self._p_sleep:
                    sleep(random() * self._t_sleep)
                crawl_function = {'.info': get_user_info(item)}.get(self._extension)
                self.log(' ' + item + '...')
                try:
