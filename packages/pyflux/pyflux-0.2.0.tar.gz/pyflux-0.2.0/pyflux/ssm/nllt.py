...
class NLLT(tsm.TSM):
	...
	**** NON-GAUSSIAN LOCAL LINEAR TREND MODEL ****
	...
	def t(cls,data,integ=0,target=None):
		...
		x._param_desc.append({'name' : 'Sigma^2 irregular','index': 3, 'prior': ifr.Uniform(transform='exp'), 'q': dst.Normal(0,3)})
		x._param_desc.append({'name' : 'v','index': 4, 'prior': ifr.Uniform(transform='exp'), 'q': dst.Normal(0,3)})
	...
	def t_likelihood(self,beta,alpha):
		return np.sum(ss.t.logpdf(x=self.data,df=self._param_desc[3]['prior'].transform(beta[3]),loc=alpha,scale=self._param_desc[2]['prior'].transform(beta[2])))
	...
	def plot_fit(self,**kwargs):
		...
		if len(self.params) == 0:
			...
			print self.states_mean.shape
			print self.states_upper_95.shape
