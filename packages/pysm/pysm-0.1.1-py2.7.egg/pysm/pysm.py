...
class TransitionsContainer(object):
    ...
    def _get_transition_matching_condition(self, key, event):
        for transition in self._transitions[key]:
            if transition['condition'](transition['from_state'], event) is True:
                return transition
...
class StateMachine(State):
    ...
    def dispatch(self, event):
        ...
        if to_state is None:
            transition['action'](from_state, event)
            return

        transition['before'](from_state, event)
        top_state = self._exit_states(event, from_state, to_state)
        transition['action'](to_state, event)
