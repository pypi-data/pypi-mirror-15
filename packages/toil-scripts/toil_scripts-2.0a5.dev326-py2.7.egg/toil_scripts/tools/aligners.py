...
def run_bwakit(job, config, threads, sort=True, trim=False):
    ...
    if getattr(config, 'alt', None):  # If an alt file was provided
    ...
    # Read groups
    if getattr(config, 'rg_line', None):  # If a read group line was provided
        rg = config.rg_line
    else:  # Generate a read group line to place in the BAM.
