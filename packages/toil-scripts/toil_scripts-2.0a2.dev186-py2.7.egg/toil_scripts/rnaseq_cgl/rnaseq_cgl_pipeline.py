...
def pipeline_declaration(job, config, cutadapt_output):
    """
    Define pipeline edges that use the fastq files

    :param JobFunctionWrappingJob job: passed automatically by Toil
    :param Namespace config: Argparse Namespace object containing argument inputs
    :param tuple(str, str, bool) cutadapt_output: R1 FileStoreID, R2 FileStoreID, Improper Pairing (True/False)
    """
    r1, r2 = cutadapt_output
    ...
    if config.kallisto_index:
        ...
        kallisto_output = job.addChildJobFn(run_kallisto, config.cores, r1, r2, config.kallisto_index,
    ...
    if config.star_index and config.rsem_ref:
        ...
        # rsem_output = job.addChildJobFn(star_alignment, config, r1, r2).rv()
    job.addFollowOnJobFn(consolidate_output, config, kallisto_output, rsem_output)


def star_alignment(job, config, r1, r2):
    job.fileStore.logToMaster('Queueing RSEM job for: ' + config.uuid)
    mem = '2G' if config.ci_test else '40G'
    star = job.addChildJobFn(run_star, config.cores, r1, r2, star_index_url=config.star_index,
...
def rsem_quantification(job, config, star_output):
    ...
    return job.addFollowOnJobFn(rsem_postprocessing, config, rsem_output)
...
def main():
    ...
    # Pipeline execution
        ...
        require(config.output_dir or config.s3_dir, 'output-dir OR s3-dir need to be defined, otherwise nothing to do.')
        # Program checks
