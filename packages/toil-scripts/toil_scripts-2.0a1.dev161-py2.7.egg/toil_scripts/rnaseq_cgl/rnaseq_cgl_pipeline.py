...
def parse_samples(path_to_manifest=None, sample_urls=None):
    ...
    if sample_urls:
        for url in sample_urls:
            samples.append(['tar', 'paired', os.path.basename(url), url])
    elif path_to_manifest:
        with open(path_to_manifest, 'r') as f:
            for line in f.readlines():
                if not line.isspace() and not line.startswith('#'):
                    sample = line.strip().split('\t')
                    require(len(sample) == 4, 'Bad manifest format! Expected 4 cols, got: {}'.format(sample))
...
def generate_config():
    return textwrap.dedent("""
    {
        ...
        'rsem-ref': '',         # Required: URL (http, file, s3) to reference tarball used by RSEM.for
        ...
        'ssec': None,           # Optional: Provide a path to a 32-byte key used for SSE-C Encryption in Amazon
        'gt-key': None,         # Optional: Provide a path to a CGHub Key that has access to the TCGA
