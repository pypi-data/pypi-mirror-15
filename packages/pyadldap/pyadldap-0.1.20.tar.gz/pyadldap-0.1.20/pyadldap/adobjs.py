...
#			raise self.adldap.NotDNDefinied("Not DN Definied")
		...
		if not isNew: #is modify action
			...
			if attrs.get('addnewmember',False):
				if attrs['addnewmember'].__dict__.get('is_modified',False):
					modtype= ldap.MOD_ADD
					modlist.append((modtype,'member',[attrs['addnewmember'].value]))
				del attrs['addnewmember']
				del self.addnewmember
