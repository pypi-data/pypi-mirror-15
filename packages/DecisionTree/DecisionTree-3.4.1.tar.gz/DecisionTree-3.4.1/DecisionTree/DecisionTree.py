...
__date__    = '2016-May-1'
__url__     = 'https://engineering.purdue.edu/kak/distDT/DecisionTree-3.4.1.html'
...
@endcode
    ...
    coefficients are calculated using just the linear least-squares method
    and no further refinement of the regression coefficients is carried out
    using gradient descent.  This is the fastest way to calculate the
    regression coefficients.  When this parameter is set to 1, you get a
    weak version of gradient descent in which the Jacobian is set to the
    "design matrix" itself.  You are likely to get the best results when
    this parameter is set to 2 --- however at a cost of much longer
    computation time.  My tutorial titled "Linear Regression and Regression
    Trees" (at the URL shown previously) has a more complete explanation
    for these three choices for the parameter jacobian_choice.
