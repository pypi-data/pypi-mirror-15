...
class PlacFormatter(argparse.RawDescriptionHelpFormatter):
    def _metavar_formatter(self, action, default_metavar):
        ...
        action.choices = dict((n, c) for n, c in list(choices.items())
                              if not n.startswith('.'))
...
class TaskManager(object):
    ...
    def close(self):
        ...
        for task in list(self.registry.values()):
            try:
    ...
    def _get_latest(self, taskno=-1, status=None):
        ...
        if status:
            tasks = [t for t in list(self.registry.values())
                     if t.status == status]
        else:
            tasks = [t for t in list(self.registry.values())]
...
class Manager(StartStopObject):
    ...
    def start(self):
        ...
        for monitor in list(self.registry.values()):
            monitor.start()
        self.started = True
...
class Interpreter(object):
    ...
    def tasks(self):
        ...
        return list(self.tm.registry.values())
    ...
    def _parse_doctest(self, lineiter):
        ...
        return list(zip(inputs, outputs, positions))
