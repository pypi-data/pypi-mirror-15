...
def getFilter(filter_selected, filters):
	for f in filters:
		if f[0] == filter_selected:
			return f
...
def filterData(queryset,filters,set_filters,filter_by_date=True):
	...
	if len(set_filters.items()) > 0:
		for filt in filters:
			if filt[1] == 'date_range' and filter_by_date:
				...
				if filt[0] in set_filters and set_filters[filt[0]][1] not in ('',None,'None','NULL'):
					clean_filters[set_filters[filt[0]][0]] = set_filters[filt[0]][1]
	queryset = queryset.filter(**clean_filters)
	return queryset

def filterDataOld(queryset,filters,set_filters,filter_by_date=True):
	clean_filters = {}
	if len(set_filters.items()) > 0:
		for filter in filters:
			if filter[1] == 'date_range' and filter_by_date:
...
def gridData(queryset, request,footerData='false'):
	...
	# Rows to return
	if rows>0:
		total = max(1, int(ceil(float(records)/float(rows))))
	else:
		total=1
