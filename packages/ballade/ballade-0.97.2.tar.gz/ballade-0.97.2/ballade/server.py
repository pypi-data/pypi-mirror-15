    ...
    def on_header(self, header_buffer):
        ...
        logging.info(self.client_ip + ":" + str(self.client_port) + " -> " +
                     " ".join([self.method.decode(),
                               ...
                               self.version.decode()]))
        if self.auth:
            if header.get("proxy-authorization") and self.auth == \
                    header.get("proxy-authorization").split()[1]:
                pass
            else:
        ...
        if self.method == b"CONNECT":
            ...
            if header.get("proxy-connection").lower() == "close" or header.get("connection").lower() == "close":
                close_connection = True
            elif self.version == b"HTTP/1.0" and (header.get("proxy-connection").lower() == "keep-alive" or header.get(
                    "connection").lower() == "keep-alive"):
                close_connection = False
            elif self.version == b"HTTP/1.1":
                close_connection = False
            for k in ["proxy-connection", "proxy-authenticate", "connection"]:
                if header.get(k):
