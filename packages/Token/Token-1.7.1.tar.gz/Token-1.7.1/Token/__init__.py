...
def help():
    ...
    print("| create_access(client, account, pk, session_id)              |")
    print("+-------------------------------------------------------------+")
    print("| COMMANDS - Provider Service                                 |")
    print("| create_member(device_name, push_id, pk)                     |")
    ...
    print("| create_device(alias, device_name, push_id, pk)              |")
    print("+-------------------------------------------------------------+")
...
class ProviderService(CPA):
    ...
    def create_device(self, alias, device_name, push_id, pk):
        request = {'name':device_name, 'pushNotificationId':push_id, \
            'publicKeys':[pk]}
        return self.create_alias_device_route(alias, request)
    ...
    def create_account(self, bank_code, access_id, name):
        payload = context["subject_id"].split('device-')[-1]
...
class BankService(CBA):
    ...
    def extra(self):
        print("extra")
    def create_access(self, client, account, public_key, session_id='test'):
        request = {
...
try:
    with open('config.json') as json_data_file:
        ...
        if "providers" in json_data: providers = json_data["providers"]
        if "banks" in json_data: banks = json_data["banks"]
        if "clients" in json_data: clients = json_data["clients"]
        if "accounts" in json_data: accounts = json_data["accounts"]
...
def set_context(data, sk):
    ...
    if hasattr(data, 'device'):
        subject_id = "provider-" + data.provider + "-device-" + data.device.id
    else:
        subject_id = data
    context = {
