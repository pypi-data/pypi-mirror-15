...
class FaxApi(object):
    ...
    def put_documents_document_id(self, document_id, **kwargs):
        """
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.put_documents_document_id(document_id, callback=callback_function)

        :param callback function: The callback function
            for asynchronous request. (optional)
        :param str document_id: Document ID (required)
        :param FaxDocument body: Document
        :return: FaxDocument
                 If the method is called asynchronously,
                 returns the request thread.
        """
        ...
        for key, val in iteritems(params['kwargs']):
            if key not in all_params:
                raise TypeError(
                    "Got an unexpected keyword argument '%s'"
                    " to method put_documents_document_id" % key
                )
            params[key] = val
        del params['kwargs']
        ...
        # verify the required parameter 'document_id' is set
        if ('document_id' not in params) or (params['document_id'] is None):
            raise ValueError("Missing the required parameter `document_id` when calling `put_documents_document_id`")
