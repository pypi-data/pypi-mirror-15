...
from pythonsol.MemoryCache import MemoryCache
from pythonsol.RedisCache import RedisCache

logger = logging.getLogger("HighCache")
...
class HighCache(object):
    ...
    def stop_cache(self):
        ...
        s = self._memory_cache._is_started
        if s:
            logger.info("_memory_cache : _is_started=%s, forcing stop_cache now", s)
            self._memory_cache.stop_cache()

        # noinspection PyProtectedMember
        s = self._redis_cache._is_started
        if s:
            logger.info("_redis_cache : _is_started=%s, forcing stop_cache now", s)
