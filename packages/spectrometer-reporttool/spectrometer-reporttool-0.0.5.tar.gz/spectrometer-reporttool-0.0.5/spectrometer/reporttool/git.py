...
class GitReport():
    ...
    def fetch_data(self):
        ...
        self.authors = Counter(commit['author_email'] for commit in self.commits)
        self.organizations = Counter(commit['author_email'].split('@', 1)[-1] for commit in self.commits)

        for c in self.commits:
            _offset = c['author_tz_offset']
            if _offset > 0:
                _time = time.strftime('-%H%M',time.gmtime(_offset))
            else:
                _time = time.strftime('+%H%M',time.gmtime(abs(_offset)))
            self.tz_data['UTC{0}'.format(_time)] += 1

            result = re.findall('(?:Also-By|Co-Authored-By): (.+) <(.+)>', c['message'])
            for name, email in result:
                self.authors[email] += 1
                self.organizations[email.split('@', 1)[-1]] += 1

        # Replace emails with author names
        keys = self.authors.keys()
        _dict = OrderedDict()
        for email in keys:
            for commit in self.commits:
                if commit["author_email"] == email:
                    _dict['{0}:{1}'.format(commit['author'], commit['author_email'])] = self.authors[email]
                    break
    ...
    def print_report(self):
        ...
        top_authors = ['{0} ({1})'.format(k.split(':')[0], v) for k, v in OrderedDict(self.authors.most_common(10)).items()]
