from .msg import KDelPayload,KGetPayload
...
class Riak(Base):
    ...
    def __services__(self):
        def on_del(_):
            ...
            self.channel['del'] = _.result().channel
            self.active = True
            return True
        ...
        self.rmq.channel('riak_del_%s' % self.uuid,self.ioengine.future(on_del))
    ...
    def del(self,cluster,bucket,key):
        try:
            assert self.active
        except AssertionError:
            def callback(_):
            ...
            self.channel['del'].basic_publish(
