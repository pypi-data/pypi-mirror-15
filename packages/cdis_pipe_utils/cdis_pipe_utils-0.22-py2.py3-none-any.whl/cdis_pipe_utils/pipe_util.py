...
def get_connect_dict(db_cred_s3url = None, s3cfg_path = None, ini_section='DEFAULT', db_cred_path = None, logger = None):
    if db_cred_path is None:
        if (db_cred_s3url is None) or (s3cfg_path is None):
            logger.debug('Must define db_cred_s3url and s3cfg_path whe db_cred_path is None')
            sys.exit(1)
    ...
    if db_cred_path is not None: # only remove when super concerned/fast
