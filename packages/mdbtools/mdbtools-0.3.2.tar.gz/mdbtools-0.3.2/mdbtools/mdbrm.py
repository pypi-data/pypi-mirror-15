...
class mdbrm(MDBtool):
    ...
    def __init__(self):
        ...
        cli.add_argument('collections',type=str, nargs=cli.ALL_REMAINING_ARGS,
            help="comma-separated name(s) of collections to remove")
    ...
    def parse_and_confirm(self):
        ...
        if not collections:
            print "Please specify one or more collections to delete."
            sys.exit(101)
        ...
        question = "\nAbout to delete collections\n\t%s" \
                "\nfrom server::database %s::%s\n" % \
    ...
    def perform_delete(self):
        ...
        for coll in self.options.collections:
            if self.collectionExists(coll):
                ...
                print "In %s::%s this collection does not exist: %s" \
