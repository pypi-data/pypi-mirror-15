...
class mdbquery(MDBtool):
    ...
    def __init__(self):
        ...
        cli.add_argument('-c', '--chunksize', default=50, type=int,
                    help="Change the chunk/page size (limit # of records "+ \
                    "returned; 0 == 'unlimited') [default: %(default)s]")
        ...
        cli.add_argument('-l', '--list', default=False, action='store_true',
                    help='list documents as they are retrieved (implicitly '+
                         'turns on --retrieve option) [default: %(default)s]')
        cli.add_argument('-n', '--numiters', default=1, type=int,
                    help='perform query N times [default: %(default)s]')
        ...
        cli.add_argument('queries',
                    help='one or more query expressions, in the form of:: '+ \
    ...
    def normalize(self, results):
        ...
        value  = results["executionStages"]["inputStage"].get("inputStage", None)
        if value and value['stage'] == 'IXSCAN':
            normed['index'] = value['indexName']
        else:
            normed['index'] = 'none'
    ...
    def do_query(self, collection, terms, hint=0):
        ...
        if hint:
            ...
            cursor = collection.find(SON_terms).hint( [(terms[hint-1][0], pymongo.ASCENDING)] ).limit(opts.chunksize)
            hint = str(hint > 0) + ":%d" % hint
        else:
            #cursor = collection.find( SON(terms)).limit(opts.chunksize)
            cursor = collection.find( SON_terms, limit=opts.chunksize)
            hint = False
        ...
        if opts.explain:
            ...
            results = self.normalize( results["executionStats"] )
    ...
    def do_queries(self):
        ...
        if os.path.isfile(queries):
            queries = open(queries, "rb").read().split('\n')
        else:
            queries = queries.split('|')

        # Multiple queries can be specified at CLI or in file, separated by |
        for q in queries:
            # Partition each query into Collection:key=value [,key=value ...]
            q = q.split(':')
            if len(q) < 2:
                q.append('')
            for _ in range(opts.numiters):
                self.do_query(q[0], parse_keyvals( q[1] ), opts.Hint)
