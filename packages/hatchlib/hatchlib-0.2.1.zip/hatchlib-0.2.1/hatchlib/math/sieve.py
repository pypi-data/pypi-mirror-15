...
def atkin(lim):
	...
	for a in range(1,s_lim):
		for b in range(1,s_lim):
			n=4*x**2+y**2
			if n<=limit and (n%12==1 or n%12==5): isprime[n]=not isprime[n]
			
			n=3*x**2+y**2
			if n<=limit and n%12==7: isprime[n]=not isprime[n]
			
			n=3*x**2-y**2
			if x>y and n<=limit and n%12==11: isprime[n]=not isprime[n]
