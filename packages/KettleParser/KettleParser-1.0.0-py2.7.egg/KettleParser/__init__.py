...
class ParseKettleXml(object):
    ...
    def _parse_steps(self):
        ...
        try:
            for step in self.root.iter(self.step_node):
                try:
                    self.steps[self._get_text(step, "name")] = {"name": self._get_text(step, "name"),
    ...
    def _parse_hops(self):
        ...
        try:
            for hop in self.root.iter("hop"):
                ...
                _step_from = self.steps[self._get_text(hop, "from")]
                _step_to = self.steps[self._get_text(hop, "to")]
    ...
    def _build_graph(self):
        ...
        for hop in self.hops:
            if not hop["enabled"]:
                continue
            if hop["from"]["name"] not in self.graph:
                self.graph[hop["from"]["name"]] = [hop["to"]["name"]]
            else:
                self.graph[hop["from"]["name"]].append(hop["to"]["name"])
        return self.graph
    ...
    def get_enabled_steps(self):
        ...
        for step in enabled_hops:
            enabled_from.append(step["from"]["name"])
            enabled_to.append(step["to"]["name"])
    ...
    def get_disabled_steps(self):
        ...
        for step in disabled_hops:
            disabled_from.append(step["from"]["name"])
            disabled_to.append(step["to"]["name"])
