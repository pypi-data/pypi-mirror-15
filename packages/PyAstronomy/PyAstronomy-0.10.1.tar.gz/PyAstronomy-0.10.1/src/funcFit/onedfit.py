...
class OneDFit(_OndeDFitParBase, _PyMCSampler):
  ...
  def fitEMCEE(self, x=None, y=None, yerr=None, nwalker=None, priors=None, pots=None, scales=None, \
               sampleArgs=None, dbfile="chain.emcee", ps=None, emcp=None, toMD=True, custlnpost=None):
    """
    ...
    if custlnpost is None:
      # Data completeness is only relevant if no custom posterior is defined.
      ...
      # Custom posterior is provided. Data may still be relevant, but completeness
      # is not an issue (which could be identified here).
      self._fufDS = FufDS(x, y, yerr)
    ...
    if custlnpost is None:
      # No custom posterior probability distribution is given
      # Use, -0.5 ln(chisqr)
      ...
      # Custum posterior probability distribution has been
      # specified.
      
      def lnpostdf(values):
        # Parameter-Value dictionary (containing only names and values
        # of free parameters)
        ps = dict(zip(fpns, values))
        # Update the parameter values of the object
        self.assignValues(ps)
        # Update the model
        self.updateModel()
        # Calculate natural logarithm of posterior probability
        # distribution using the specified function 
        lnpdf = custlnpost(self)
        return lnpdf
  ...
  def exposeData(self):
    """
      Expose the currently used data set.
      
      Returns
      -------
      Data : FuFDS object
          The data currently used by the object.
    """
    return self._fufDS  
