jQuery(function ($) {
    ...
    var itemSelector = window.__wladmin_installed ? '.items' : 'tbody';
    var parentSelector = '.dynamic-inline ' + itemSelector;
    var orderSelector = window.__wladmin_installed ? '._order input' : '.field-_order input';

    // Apply drag and drop to orderable inlines.
    $(parentSelector).sortable({
        handle: '.ordering', axis: 'y', opacity: '.7',
    ...
    $(addAnother).click(function () {
        ...
        var rows = getRows();
        $(rows[0]).show();
        // WlAdmin's inline header for tabular inlines is
        // actually part of the selector, so for it we run this twice.
        if (window.__wladmin_installed && $(rows[0]).hasClass('legend')) {
            $(rows[1]).show();
        }
        if (getRows().length === 0) {
            $(this).hide();
