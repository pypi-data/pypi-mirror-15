...
__date__    = '2016-June-23'   
__url__     = 'https://engineering.purdue.edu/kak/distWatershed/Watershed-2.0.3.html'
...
PUBLIC METHODS:
    ...
    (3)  apply_otsu_threshold_to_hue(polarity)

         Applies the optimum Otsu threshold to hue in order to separate the
         image into foreground and background.  Regarding the argument
         'polarity', if it is '-1', all pixels whose hue is below the
         optimum Otsu threshold are accepted.  On the other hand, if the
         polarity is '1', all pixels whole hue is higher than the
         calculated threshold are accepted.
    ...
    (5)  compute_Z_level_sets_for_gradient_image()
    ...
    (12) display_watershed_contours_in_color()
    ...
    (17) extract_image_region_interactively_through_mouse_clicks():
    ...
    (18) extract_image_region_interactively_by_dragging_mouse():
    ...
    (19) extract_rectangular_masked_segment_of_image(self, horiz_start, horiz_end, vert_start, vert_end)
    ...
    (20) extract_segmented_blobs_using_contours( min_length )
    ...
    (21) extract_segmented_blobs_using_region_growing( num_of_segments, min_area )
    ...
    (22) extract_watershed_contours_with_random_sampling( how_many, min_len )
    ...
    (23) extract_watershed_contours_separated()
    ...
    (24) gendata(feature, size_tuple, location_tuple, orientation_or_radius, filename)
    ...
    (25) make_binary_pic_art_nouveau( filename )
    ...
    (26) mark_image_regions_for_gradient_mods()
    ...
    (27) modify_gradients_with_marker_minima()
    ...
    (28) propagate_influence_zones_from_bottom_to_top_of_Z_levels()
...
#        self.displayImage6(Watershed.extracted_image, "Extracted_Image -- close window when done viewing")
    ...
    def apply_otsu_threshold_to_hue(self, polarity):
        '''
        ...
        hue is higher than the calculated threshold are accepted.
        '''
        assert polarity in [-1,1], "\nThe argument with which apply_otsu_threshold_to_hue() is called must be either -1 or 1 --- aborting"
        self.displayImage6(self.original_im, "input_image -- close window when done viewing")
        width,height = self.original_im.size
        how_many_pixels = width * height
        hsv_image = self.original_im.copy().convert("HSV")
