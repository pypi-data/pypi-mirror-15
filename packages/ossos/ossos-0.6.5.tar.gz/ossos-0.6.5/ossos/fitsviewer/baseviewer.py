...
class WxMPLFitsViewer(object):
    ...
    def draw_uncertainty_ellipse(self, cutout):
        ...
        try:
            ...
            predict_ra = sky_coord.ra.to(units.degree).value
            predict_dec = sky_coord.dec.to(units.degree).value
            ...
            try:
                measured_ra = cutout.reading.mpc_observation.coordinate.ra.to(units.degree).value
                measured_dec = cutout.reading.mpc_observation.coordinate.dec.to(units.degree).value
                try:
                    print "{:5.2f} {:5.2f} || {:5.2f} {:5.2f} # {}".format((predict_ra - measured_ra)*3600.0,
                                                        (predict_dec - measured_dec)*3600.0,
