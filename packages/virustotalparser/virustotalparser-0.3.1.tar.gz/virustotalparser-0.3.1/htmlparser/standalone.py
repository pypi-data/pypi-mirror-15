...
def main():
    try:
        ...
        print("Usage:\n"
              "parser html_dir json_dir\n"
              "or\n"
              "parser html_dir\n"
              "Dirs can be absolute or relative directories that contain correct json/html files\.n"
              "Json file must contain json list, eg. [\"MimeType\", \"File name\"].\n"
              "If only one argument is given (html_dir) then parser returns information only about first page.\n"
              "If two arguments are given, parser searches for information in html file about elements given in json file.\n",
              "Result is printed on stdio and written in file result.json located in the same folder as given html file.\n")
