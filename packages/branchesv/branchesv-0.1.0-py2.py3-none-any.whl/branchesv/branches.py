...
class GitBranch(object):
    ...
    *repo_url*: Urls of remote origins of the repository
        ...
        if os.path.exists(path) and os.path.isdir(path) and \
           not os.path.islink(path):
            for element in os.listdir(path):
                ...
                if os.path.isdir(element_path) and element not in ['.', '..'] \
                    and not os.path.islink(element_path):
                    if element == '.git':
    ...
    def __clone(self, path, branch):
        ...
        _logger.info('Cloning %s from: %s - %s', branch['name'], url, branch['branch'])
        try:
    ...
    def __pull(self, path, branch):
        ...
        try:
            ...
            if repo_index >= 0:
                ...
                _logger.error("Could not read from remote repository %s", repo.remotes[repo_index])
                res = utils.set_message(branch, "Could not read from remote repository")
        except git.exc.GitCommandError as error:
            res = utils.set_message(branch, error.command[0])
        except git.exc.NoSuchPathError:
            _logger.error("fatal: No Such Path %s", pull_path)
            res = utils.set_message(branch, "fatal: No Such Path {path}".format(path=pull_path))
        return res


    def pull(self, branch, path):
