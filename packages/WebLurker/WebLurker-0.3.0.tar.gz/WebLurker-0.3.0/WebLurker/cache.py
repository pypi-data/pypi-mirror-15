...
class PickleCache(BaseCache):
    ...
    def clone(self):
        return PickleCache(cache_path=self.cache_path, expiration_days=self.expiration_days.days, compression=self.compression)
    ...
    def __setitem__(self, key, value):
        try:
            ...
            try:
                withoutlockobject = self.clone()
                del withoutlockobject.mutex
                if self.compression:
                    with open(self.cache_path, 'wb') as cache_file:
                        cache_file.write(zlib.compress(pickle.dumps(withoutlockobject)))
                else:
                    with open(self.cache_path, 'wb') as cache_file:
                        cache_file.write(pickle.dumps(withoutlockobject))
            except:
                raise CacheError(
    ...
    def __getitem__(self, item):
        ...
        try:
            self.mutex.acquire()
            content = self.html_content[item]
        finally:
            self.mutex.release()
    ...
    def __contains__(self, item):
        ...
        try:
            self.mutex.acquire()
            result = item in self.html_content
        finally:
            self.mutex.release()
        return result
...
class FileCache(BaseCache):
    ...
    def clone(self):
        return FileCache(cache_dir=self.cache_dir, expiration_days=self.expiration_days.days, compression=self.compression)
                ...
                if self.compression:
                    ...
                    if type(value) is not bytes:
                        ...
                        with open(path, 'wb') as file:
                            file.write(value)
                try:
                    withoutlockobj= self.clone()
                    del withoutlockobj.mutex
                    if self.compression:
                        with open(os.path.join(self.cache_dir, self.indexes_filename), 'wb') as file:
                            file.write(zlib.compress(pickle.dumps(withoutlockobj)))
                    else:
                        with open(os.path.join(self.cache_dir, self.indexes_filename), 'wb') as file:
                            file.write(pickle.dumps(withoutlockobj))
                except:
                    raise CacheError(
                        "Error when trying to save the cache to a file. Try deleting the cache directory at " + str(
                            os.path.join(self.cache_dir, self.indexes_filename)))
