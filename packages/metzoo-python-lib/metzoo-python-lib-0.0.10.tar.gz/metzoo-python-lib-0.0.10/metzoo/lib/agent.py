...
class Agent(Comparable):
    ...
    HEADERS = \
            ...
            , "User-Agent": "metzoo-python-sdk/" + __version__
            }
    ...
    def __init__(self, customer_key, name, url="http://api.metzoo.com"):
        ...
        if customer_key is None or len(customer_key) == 0 or name is None:
            raise AgentCreationError(500, "You must provide a customer key and a name")
    ...
    def __check_numeric_type(self, t, name):
        if t not in [int, float, long]:
            raise MetzooInvalidData(got=t, expected="{}, {} or {} for {}".format(int, float, long, name))
