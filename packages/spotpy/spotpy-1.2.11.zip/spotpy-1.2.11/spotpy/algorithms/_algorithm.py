...
class _RunStatistic(object):
    ...
    def __call__(self,rep,objectivefunction,params,acttime,startime,repetitions):
        #get str showing approximate timeleft to end of simulation in H, M, S 
        self.timestr = time.strftime("%H:%M:%S", time.gmtime(round(((acttime-starttime)/
                           (rep + 1))*(repetitions-(rep + 1 )))))
        if type(objectivefunction)==type([]):
            if objectivefunction[0]>self.objectivefunction_max:
                self.objectivefunction_max = objectivefunction[0]#Show only the first best objectivefunction when working with more than one objectivefunction
                self.params_max = params
                self.rep_max = rep
            if objectivefunction[0]<self.objectivefunction_min:
                self.objectivefunction_min = objectivefunction[0]#Show only the first best objectivefunction when working with more than one objectivefunction
                self.params_min = params
                self.rep_min = rep
        else:
            if objectivefunction>self.objectivefunction_max:
                self.params_max = params
                self.objectivefunction_max = objectivefunction
                self.rep_max = rep
            if objectivefunction<self.objectivefunction_min:
                self.params_min = params
                self.objectivefunction_min = objectivefunction
                self.rep_min = rep
            
            return True
        return False
    def __repr__(self):
        return 'Objectivefunction: Min: '+str(self.objectivefunction_min)+' Max: '+str(self.objectivefunction_max)
