...
class mc(_algorithm): 
    ...
    def sample(self, repetitions):
        ...
        for rep,randompar,simulations in self.repeat(param_generator):        
            ...
            like        = self.objectivefunction(simulations,self.evaluation)
            acttime=time.time()
            self.status(rep,like,randompar,acttime,startime,repetitions)
            #Save everything in the database
            self.datawriter.save(like,randompar,simulations=simulations)
            
            #Progress bar
            if acttime-intervaltime>=2:            #Refresh progressbar every second
                text='%i of %i (min obj=%g max obj=%g) est. time remaining: %s' % (rep,repetitions,
                     self.status.objectivefunction_min,self.status.objectivefunction_max,self.status.timestr)
                print(text)
        ...
        text='%i of %i (min obj=%g, max obj=%g)' % (self.status.rep,repetitions,self.status.objectivefunction_min,self.status.objectivefunction_max)
        print(text)
