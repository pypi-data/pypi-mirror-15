...
class nb_adult(Variable):
    ...
    def function(self, simulation, period):
        ...
        return period, 2 * mariage_ou_pacs + 1 * (celibataire_ou_divorce | veuf)
...
class mariage_ou_pacs(Variable):
    column = BoolCol
    entity_class = FoyersFiscaux
...
class rev_cat_rvcm(DatedVariable):
    ...
    def function_20020101_20041231(self, simulation, period):
        ...
        b12 = min_(f2ch, rvcm.abat_assvie * (1 + mariage_ou_pacs))
        TOT1 = f2ch - b12  # c12
        ...
        h12 = rvcm.abatmob * (1 + mariage_ou_pacs)
        TOT2 = max_(0, rev - h12)
        # i121= -min_(0,rev - h12)
...
class ir_plaf_qf(Variable):
    ...
    def function(self, simulation, period):
        ...
        B1 = plafond_qf.celib_enf * aa1 + plafond_qf.mariage_ou_pacs * aa2
        # tous les autres
        B2 = plafond_qf.mariage_ou_pacs * aa0  # si autre
        ...
        # marié pacs
        condition62cab = (mariage_ou_pacs | jeune_veuf) & caseS & not_(caseP | caseF)
...
class nbptr(Variable):
    ...
    def function(self, simulation, period):
        ...
        return period, (mariage_ou_pacs | jeune_veuf) * m + (veuf & not_(jeune_veuf)) * v + celibataire_ou_divorce * c
...
class ppe_elig(Variable):
    ...
    def function(self, simulation, period):
        ...
        seuil = (veuf | celibataire_ou_divorce) * (ppe.eligi1 + 2 * max_(nbptr - 1, 0) * ppe.eligi3) \
                + mariage_ou_pacs * (ppe.eligi2 + 2 * max_(nbptr - 2, 0) * ppe.eligi3)
...
class ppe_brute(Variable):
    ...
    def function(self, simulation, period):
        ...
        ligne2 = mariage_ou_pacs & xor_(basevi >= ppe.seuil1, baseci >= ppe.seuil1)
    ...
    #                           (cond2 & (base > ppe.seuil4) & (base <= ppe.seuil5)) * (ppe.seuil5 - base) * ppe.taux3)
        ...
        maj_pac = ppe_elig * (eliv | elic) * (
            (ligne1 & mariage_ou_pacs & ((ppev + ppec) != 0) & (min_(basev, basec) <= ppe.seuil3)) * ppe.pac
            * (nb_pac_ppe + nbH * 0.5)
