...
class aidper(DatedVariable):
    ...
    def function_20060101_20091231(self, simulation, period):
        ...
        max0 = P.max * (1 + mariage_ou_pacs) + P.pac1 * nb_pac2
...
class inthab(DatedVariable):
    ...
    def function_20070101_20071231(self, simulation, period):
        ...
        max0 = P.max * (mariage_ou_pacs + 1) * (1 + invalide) + nb_pac2 * P.add
...
class jeunes_ind(Variable):
    ...
    def function(self, simulation, period):
        ...
        mariage_ou_pacs_holder = simulation.compute('mariage_ou_pacs', period)
        ...
        mariage_ou_pacs = self.cast_from_entity_to_roles(mariage_ou_pacs_holder)

        elig = (age < P.age) * (rfr < P.rfr_plaf * (mariage_ou_pacs * P.rfr_mult + not_(mariage_ou_pacs)) + max_(0, nbptr - 2) * .5 *
...
class prlire(Variable):
    ...
    def function(self, simulation, period):
        ...
        plaf_resid = max_(_P.ir.rvcm.abat_assvie * (1 + mariage_ou_pacs) - f2ch, 0)
...
class quaenv(DatedVariable):
    ...
    def function_20050101_20051231(self, simulation, period):
        ...
        n = nb_pac2
        max0 = P.max * (1 + mariage_ou_pacs) + P.pac1 * (n >= 1) + P.pac2 * (n >= 2) + P.pac2 * (max_(n - 2, 0))
