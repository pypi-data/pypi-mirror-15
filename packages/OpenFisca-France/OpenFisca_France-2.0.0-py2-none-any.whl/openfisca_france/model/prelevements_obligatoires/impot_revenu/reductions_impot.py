...
class cappme(DatedVariable):
    ...
    def function_20090101_20101231(self, simulation, period):
        ...
        seuil = P.seuil * (mariage_ou_pacs + 1)
        seuil = P.seuil_tpe * (mariage_ou_pacs + 1) * (f7cu > 0) + P.seuil * (mariage_ou_pacs + 1) * (f7cu <= 0)
        return period, P.taux * min_(base, seuil)
    ...
    def function_20120101_20121231(self, simulation, period):
        ...
        base = f7cl + f7cm + f7cn
        seuil1 = P.seuil * (mariage_ou_pacs + 1)
        seuil2 = max_(0, P.seuil_tpe * (mariage_ou_pacs + 1) - min_(base, seuil1) - min_(f7cq, seuil1) - min_(f7cu, seuil1))
        seuil3 = min_(P.seuil_tpe * (mariage_ou_pacs + 1) - min_(base, seuil1) - min_(f7cq, seuil1), seuil1)
    ...
    def function_20130101_20131231(self, simulation, period):
        ...
        base = f7cl + f7cm
        seuil1 = P.seuil * (mariage_ou_pacs + 1)
        seuil2 = max_(0, P.seuil_tpe * (mariage_ou_pacs + 1) - min_(base, seuil1) - min_(f7cn, seuil1) - min_(f7cu, seuil1))
        seuil3 = min_(P.seuil_tpe * (mariage_ou_pacs + 1) - min_(base, seuil1) - min_(f7cq, seuil1), seuil1)
...
class invfor(DatedVariable):
    ...
    def function_20090101_20091231(self, simulation, period):
        ...
        return period, P.taux * (min_(f7un, P.seuil * (mariage_ou_pacs + 1)) + min_(f7up, P.ifortra_seuil * (mariage_ou_pacs + 1)) +
                min_(f7uq, P.iforges_seuil * (mariage_ou_pacs + 1)))
    ...
    def function_20100101_20101231(self, simulation, period):
        ...
        return period, (P.taux * (
            min_(f7un, P.seuil * (mariage_ou_pacs + 1)) +
            min_(f7up + f7uu + f7te, P.ifortra_seuil * (mariage_ou_pacs + 1)) +
            min_(f7uq, P.iforges_seuil * (mariage_ou_pacs + 1))))
    ...
    def function_20110101_20111231(self, simulation, period):
        ...
        max0 = max_(0, P.ifortra_seuil * (mariage_ou_pacs + 1) - f7ul)
        ...
        return period, (P.taux * (
            ...
            P.taux_ass * min_(f7ul, P.ifortra_seuil * (mariage_ou_pacs + 1)))
...
class invlst(DatedVariable):
    ...
    def function_20040101_20041231(self, simulation, period):
        ...
        seuil1 = P.seuil1 * (1 + mariage_ou_pacs)
        seuil2 = P.seuil2 * (1 + mariage_ou_pacs)
        seuil3 = P.seuil3 * (1 + mariage_ou_pacs)
...
class invrev(Variable):
    ...
    def function(self, simulation, period):
        ...
        return period, (P.taux_gs * min_(f7gs, P.seuil_gs * (1 + mariage_ou_pacs)) / 4 +
                 P.taux_gu * min_(f7gu, P.seuil_gu * (1 + mariage_ou_pacs)) / 4 +
                 P.taux_xg * min_(f7xg, P.seuil_xg * (1 + mariage_ou_pacs)) / 4 +
...
class sofipe(Variable):
    ...
    def function(self, simulation, period):
        ...
        max1 = min_(P.max * (mariage_ou_pacs + 1), P.base * rbg_int)  # page3 ligne 18
