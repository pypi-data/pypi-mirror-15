...
def run_engine(PDF=True, IMD=True, B=True, BA=True, IA=True, 
    ...
    if PDF:
        C = PairDistributionConstraint(engine=None, experimentalData=expPath, weighting="atomicNumber")
        ENGINE.add_constraints(C)
    # create and define molecular constraints
    if IMD:
        C = InterMolecularDistanceConstraint(engine=None, defaultDistance=1.5)
        ENGINE.add_constraints(C)
    if B:
        C = BondConstraint(engine=None)
        ENGINE.add_constraints(C)
    ...
    if BA:
        C = BondsAngleConstraint(engine=None)
        ENGINE.add_constraints(C)
        C.create_angles_by_definition( anglesDefinition={"THF": [ ('O'  ,'C1' ,'C4' , 95 , 135),
                                                                  ...
                                                                  ('C4' ,'H42','C3' , 80, 123) ] })
    if IA:
        C = ImproperAngleConstraint(engine=None)    
        ENGINE.add_constraints(C)
