...
grid_arrange_shared_legend = function(plot_list, title, plot_file_prefix, plot_file_format=c("pdf", "png"), width=18, height=30) {
  ...
  for (plot_type in plot_file_format){
    ...
    if (plot_type=="svg"){svg(paste0(plot_file_prefix, ".svg"), width=width/2.56, height=height/2.56)}
      
    p = plot_list[[1]] + theme(legend.position="bottom") 
    g = ggplotGrob(p)$grobs
    legend = g[[which(sapply(g, function(x) x$name) == "guide-box")]]
    lheight = sum(legend$height)

    plot = arrangeGrob(
      do.call(arrangeGrob, lapply(plot_list, function(x)
        x + theme(legend.position="none"))),
      legend,
      ncol = 1,
      #heights = unit.c(unit(1, "npc") - lheight, lheight),
      heights = c(20, 1),
      top=textGrob(title,gp=gpar(fontsize=20,font=2), vjust=0.7))
