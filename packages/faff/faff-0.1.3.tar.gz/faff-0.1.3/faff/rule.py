...
class Rule(object):
    ...
    class _Rule(object):
        ...
        Internal rule class.
        '''
        # Internal rule error messages.
        E1 = '`depends` is not an instance of `Target` class or a list'
        ...
        def __init__(self, *args, **kwargs):
            ...
            if isinstance(rule_depends, Target):
                ...
                core.raise_exception(self.E1, cls=InvalidTargetError,)
            ...
            for option in self._rule_options:
                assert Option.is_option(option), 'TODO: not an Option'
        ...
        def __call__(self):
            ...
            if total == 0:
                if self._call_function(args=self._args):
                    # Assume target updated (null target).
                    return (True, 1, 1)
                else:
                    # Assume not updated if error condition.
                    return (False, 0, 1)
            ...
            for i, target_depends in enumerate(target_depends_pairs):
                ...
                # If update required, call rule function.
                if upd:
                    if not self._call_function(target, depends, self._args):
                        # Early return caused by error condition.
                        return (False, updated, total)
                    else:
                        updated += 1
            ...
            # Return success, number of updated targets and total.
            return (True, updated, total)

        def _call_function(self, target=None, depends=None, args=None):
            '''
            Call internal rule function with target and dependencies.
            '''
            success = False
            emsg = None

            try:
                self._func(target, depends, args)
            except KeyboardInterrupt:
                emsg = 'keyboard interrupt'
            except subprocess.CalledProcessError as e:
                # Subprocess error during function call, send output to stderr.
                emsg = e.output
                ...
                # TODO: Write error to core log?
                if emsg is not None:
                    core.write_stderr(emsg)
