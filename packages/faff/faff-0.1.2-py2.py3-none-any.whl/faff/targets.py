...
class Target(object):
    ...
    @classmethod
    def set_vars(cls, target, depends):
        '''
...
class FileTarget(Target):
    ...
    def __str__(self):
        ...
        if self._ext is not None:
            pre, ext = os.path.splitext(self._args[-1])
            if self._ext_orig is None:
                self._ext_orig = ext
            self._args[-1] = pre + self._ext

        # Apply variables to arguments when evaluated as string.
        self._args = [(Variable.expand_vars(arg)) for arg in self._args]
        return os.path.join(*self._args)
    ...
    def get_orig_ext(self):
        '''
        Get original file extension of target file.
        '''
