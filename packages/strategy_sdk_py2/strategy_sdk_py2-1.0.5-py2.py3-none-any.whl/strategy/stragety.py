...
from strategy.bean.bean import StrategyOrderDetail, StrategyOrderError, StrategyError, StrategyAccountDetail, StrategyPositionStatics, \
    StrategyRequestOrder, \
    StrategyConnectionInfo, StrategyConfiguration, StrategyQuoteDataRequest
from strategy.bean.databean import FutureMarketBean, StockMarketBean, KlineBean
...
class BaseStrategy(AbstractStrategy, AbstractNotifyMessageHandler, AbstractAsyncMessageHandler):
    ...
    def req_stock_tick(self, symbol=None, begin_date=-1, begin_time=-1, end_date=-1, end_time=-1):
        req_data = StrategyQuoteDataRequest(market="", symbol=symbol, dateType=TICK_LEVEL_TWO_STOCK, beginDate=begin_date,endDate=end_date, beginTime=begin_time, endTime=end_time)
    ...
    def req_future_tick(self, symbol=None, begin_date=-1, begin_time=-1, end_date=-1, end_time=-1, auto_fill=0):
        req_data = StrategyQuoteDataRequest(market="", symbol=symbol, dateType=TICK_LEVEL_TWO_FUTURE,
                                           beginDate=begin_date, endDate=end_date, beginTime=begin_time, endTime=end_time,
                                           autoFill=auto_fill)
        return self.req_quota_data(req_data)
    ...
    def req_kline(self, symbol=None, data_type=None, cyc_def=1, begin_date=-1, begin_time=-1, end_date=-1, end_ime=-1,
        ...
        req_data = StrategyQuoteDataRequest(market="", symbol=symbol, dataType=data_type, cycCount=cyc_def,
                                           beginDate=begin_date, endDate=end_date, beginTime=begin_time, endTime=end_ime,
    ...
    def req_quota_data(self, quote_data_category):
        ...
        try:
            ...
            if response_info.is_success():
                if data_type == TICK_LEVEL_ONE_STOCK or data_type == TICK_LEVEL_TWO_STOCK:
                    result = utils_bean.json_load_bean_list(data, StockMarketBean)
                elif data_type == TICK_LEVEL_ONE_FUTURE or data_type == TICK_LEVEL_TWO_FUTURE:
    ...
    #         err = self._new_500_error()
    #     return result, err
    def req_future_order(self, symbol=None,operation=None,price_type=None,price=None,volume=None,strategy_id=None,hedge_type=HedgeFlagType.HEDGE_FLAG_HEDGE):
        if strategy_id is None:
            strategy_id = self.strategyConnectionInfo.strategyId
        return self.req_order(symbol,operation,price_type,price,volume,strategy_id,hedge_type,InterfaceType.INTERFACE_FUTURE)
    ...
    def req_order(self, symbol,operation,price_type,price,volume,strategy_id,interface_type):
        order=StrategyRequestOrder(price=price,price_type=price_type,instrument=symbol,order_operation_type=operation,interface_type=interface_type,strategy_id=strategy_id,volume=volume)
    ...
    def handle_async_message(self, async_message):
        def notify_st_subscribe_handler(self, async_message):
            try:
                ...
                data = None
                if response_info.dataType == TICK_LEVEL_TWO_FUTURE or response_info.dataType == TICK_LEVEL_ONE_FUTURE:
                    data = FutureMarketBean().from_json(response_info.data)
                elif response_info.dataType == TICK_LEVEL_TWO_STOCK or response_info.dataType == TICK_LEVEL_ONE_STOCK:
...
class SimpleStrategy(AbstractStrategy, AbstractNotifyMessageHandler):
    ...
    def req_stock_order(self, symbol,operation,price_type,price,volume,strategy_id):
        return self.traderStrategy.req_order(symbol,operation,price_type,price,volume,strategy_id,InterfaceType.INTERFACE_STOCK)
    def req_order(self, symbol,operation,price_type,price,volume,strategy_id,interface_type):
