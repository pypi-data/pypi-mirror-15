...
class BaseStrategy(AbstractStrategy, AbstractNotifyMessageHandler, AbstractAsyncMessageHandler):
    ...
    def req_stock_tick(self, symbol=None, beginDate=-1, beginTime=-1, endDate=-1, endTime=-1):
        reqData = StrategyQuoteDataRequest(market="", symbol=symbol, dateType=TICK_LEVEL_TWO_STOCK, beginDate=beginDate,
    ...
    def req_future_tick(self, symbol=None, beginDate=-1, beginTime=-1, endDate=-1, endTime=-1, autoFill=0):
        reqData = StrategyQuoteDataRequest(market="", symbol=symbol, dateType=TICK_LEVEL_TWO_FUTURE,
    ...
    def req_quota_data(self, quote_data_category):
        ...
        try:
            ...
            if response_info.is_success():
                if dataType == TICK_LEVEL_ONE_STOCK or dataType == TICK_LEVEL_TWO_STOCK:
                    result = utils_bean.json_load_bean_list(data, StockMarketBean)
                elif dataType == TICK_LEVEL_ONE_FUTURE or dataType == TICK_LEVEL_TWO_FUTURE:
                    result = utils_bean.json_load_bean_list(data, FutureMarketBean)
                elif dataType < SAMPLE_DATA_END and dataType > SAMPLE_DATA_BEGIN:
    ...
    def handle_async_message(self, async_message):
        def notify_st_subscribe_handler(self, async_message):
            try:
                ...
                elif response_info.dataType > SAMPLE_DATA_BEGIN and response_info.dataType < SAMPLE_DATA_END:
