...
def handle_rest_response(requestMethod):
	def response_handler(*args, **kwargs):
		...
		def handle_response(retry_count=0):
			logging.info("Marketo Rest API returned called with args {} and kwargs {}".format(
				args, kwargs))
			...
			if resp.status_code != 200:
				...
				for error in errors:
					logging.info("Marketo Rest API returned error {} with args {} and kwargs {}".format(
						error.get('code'), args, kwargs))
					if error.get('code') == '602' and retry_count < 2:
				...
				logging.info("Marketo Rest API returned success response {} with args {} and kwargs {}".format(
...
class MarketoRest(object):
	...
	def get_lead_activities(self, activity_type_ids, next_page_token, batch_size=300, list_id=None):
		...
		return (resp['result'], resp.get('nextPageToken', None), resp.get('moreResult'))
