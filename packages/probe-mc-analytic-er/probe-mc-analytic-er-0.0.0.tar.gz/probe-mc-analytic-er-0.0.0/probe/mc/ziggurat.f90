...
MODULE Ziggurat
   ...
   REAL(8), PARAMETER  ::  m1=2147483648.0D0,   m2=2147483648.0D0,      &
                            half=0.5D0
   REAL(8)             ::  dn=3.442619855899D0, tn=3.442619855899D0,    &
                            vn=0.00991256303526217D0,                     &
                            q,                    de=7.697117470131487D0, &
                            te=7.697117470131487D0,                       &
                            ve=0.003949659822581572D0
!$OMP THREADPRIVATE(q, dn, tn, vn, de, te, ve)
   INTEGER,  SAVE       ::  iz, jz, jsr=123456789, kn(0:127),              &
                            ke(0:255), hz
!$OMP THREADPRIVATE(iz, jz, jsr, kn, ke, hz)
   REAL(8), SAVE       ::  wn(0:127), fn(0:127), we(0:255), fe(0:255)
!$OMP THREADPRIVATE( wn, fn, we, fe)
...
SUBROUTINE zigset( jsrseed )
   ...
   DO  i = 126, 1, -1
      dn = SQRT( -2.0D0 * LOG( vn/dn + EXP( -half*dn*dn ) ) )
      kn(i+1) = (dn/tn)*m1
      tn = dn
      fn(i) = EXP(-half*dn*dn)
      wn(i) = dn/m1
...
!!!
!!!   !  Local variables
!!!   REAL(DP), PARAMETER   ::  zero=0.0D0, one=1.0D0, half=0.5D0, con=1.28D0
!!!   REAL(DP)              ::  z, y
!!!   LOGICAL               ::  up
!!!
!!!   !  Machine dependent constants
!!!   REAL(DP), PARAMETER  ::  ltone = 7.0D0, utzero = 18.66D0
!!!   REAL(DP), PARAMETER  ::  p = 0.398942280444D0, q = 0.39990348504D0,   &
!!!                            r = 0.398942280385D0, a1 = 5.75885480458D0,  &
!!!                            a2 = 2.62433121679D0, a3 = 5.92885724438D0,  &
!!!                            b1 = -29.8213557807D0, b2 = 48.6959930692D0, &
!!!                            c1 = -3.8052E-8D0, c2 = 3.98064794E-4D0,     &
!!!                            c3 = -0.151679116635D0, c4 = 4.8385912808D0, &
!!!                            c5 = 0.742380924027D0, c6 = 3.99019417011D0, &
!!!                            d1 = 1.00000615302D0, d2 = 1.98615381364D0,  &
!!!                            d3 = 5.29330324926D0, d4 = -15.1508972451D0, &
!!!                            d5 = 30.789933034D0
!!!
!!!   up = upper
!!!   z = x
