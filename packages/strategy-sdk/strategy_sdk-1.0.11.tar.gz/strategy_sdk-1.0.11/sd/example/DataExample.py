# -*- coding:utf-8 -*-
from sd.strategy.bean.datatype import TICK_STOCK, TICK_FUTURE, KLINE_END, KLINE, KLINE_ONE_MINUTE, SAMPLE_DATA_MINUTE

from sd.strategy.stragety import BaseStrategy
import logging
from sd.configuration.logconfig import configure_log

logging = configure_log("dataexample.log", logging.INFO)
...
if __name__ == "__main__":
    from sd.configuration.defaultconfig import quote_config
    ...
    if not dataExample.init():
        ...
        exit(1)
    # #######订阅上期所行情
    symbol = "%d.%s.%s" % (TICK_FUTURE, "SHFE", "rb1610")
    ...
    #     logging.error("Failed to subscribe %s because of %s",symbol,ret.to_json())
    #######订阅上交所行情
    symbol = "%d.%s.%s" % (TICK_STOCK, "SHSE", "600600")
    ret = dataExample.req_subscribe(symbol)
    if ret.errorId != 0:
        logging.error("Failed to subscribe %s because of %s", symbol, ret.to_json())
    #######订阅深交所行情
