...
from sd.data.common.enum import HedgeFlagType
from sd.exception import StrategyErrorCode
from sd.exception.exception import ConnectionBrokenException
from sd.strategy.bean import protoquote
from sd.strategy.bean.bean import StrategyOrderDetail, StrategyOrderError, StrategyError, StrategyAccountDetail, \
...
from sd.strategy.bean.enum import PriceType, InterfaceType, OperationType
from sd.strategy.bean.datatype import *
from sd.strategy.client.client import StrategyClient
from sd.strategy.interface import AbstractStrategy, AbstractNotifyMessageHandler
...
class BaseStrategy(AbstractStrategy, AbstractNotifyMessageHandler, AbstractAsyncMessageHandler):
    ...
    def req_subscribe(self, data_type, symbol):
        s = "%d.%s" % (data_type, symbol)
        return self.req_subscribe(s)
        ...
        def notify_st_subscribe_handler(self, async_message):
            try:
                ...
                if response_info.dataType == TICK_FUTURE or response_info.dataType == TICK_STOCK:
                    data = utils_bean.proto_to_bean(response_info.protocolMessage.byteBody, protoquote.TickBean,
                                                    TickBean)
                elif SAMPLE_DATA_BEGIN < response_info.dataType < SAMPLE_DATA_END:
                    data = utils_bean.proto_to_bean(response_info.protocolMessage.byteBody, protoquote.KLineBean,
                                                    KlineBean)
