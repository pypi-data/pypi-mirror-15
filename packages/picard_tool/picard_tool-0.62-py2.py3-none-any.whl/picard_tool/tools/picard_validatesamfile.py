...
def picard_validatesamfile(uuid, bam_path, input_state, engine, logger):
    ...
    if pipe_util.already_step(step_dir, 'validate', logger):
        ...
        cmd = ['java', ' -Djava.io.tmpdir=`pwd`', '-d64', '-jar', os.path.join(home_dir, 'tools/picard-tools/picard.jar'), 'ValidateSamFile', 'MO=' + str(mo), 'INPUT=' + bam_path, 'OUTPUT=' + validate_file, 'VALIDATION_STRINGENCY=LENIENT', 'TMP_DIR=`pwd`']
