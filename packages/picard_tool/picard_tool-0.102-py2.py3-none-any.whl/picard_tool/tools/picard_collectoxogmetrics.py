...
def picard_collectoxogmetrics(uuid, bam_path, db_snp_path, reference_fasta_path, input_state, engine, logger):
    ...
    reference_fasta_name = os.path.basename(reference_fasta_name)
    if pipe_util.already_step(step_dir, bam_base + '_CollectOxoGMetrics', logger):
        ...
        cmd = ['java', '-d64', '-jar', os.path.join(home_dir, 'tools/picard-tools/picard.jar'), 'CollectOxoGMetrics', 'INPUT=' + bam_path, 'OUTPUT=oxoG_metrics.txt', 'DB_SNP='+ db_snp_path, 'REFERENCE_SEQUENCE=' + reference_fasta_path, 'VALIDATION_STRINGENCY=STRICT']
