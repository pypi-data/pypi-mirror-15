...
def picard_collectoxogmetrics(uuid, bam_path, db_snp_path, input_state, engine, logger):
    step_dir = os.getcwd()
    ...
    if pipe_util.already_step(step_dir, bam_base + '_CollectOxoGMetrics', logger):
        ...
        cmd = ['java', '-d64', '-jar', os.path.join(home_dir, 'tools/picard-tools/picard.jar'), 'CollectOxoGMetrics', 'INPUT=' + bam_path, 'OUTPUT=oxoG_metrics.txt', 'DB_SNP='+ db_snp_path, 'VALIDATION_STRINGENCY=STRICT']
