...
def healpix2wcs(
    ...
    ctype: str[2]
        Equivalent to the CTYPE FITS header card.
    ...
    image_size: int[2]
        ...
        If you do not provide a header, you must provide this value.
    is_noise: boolean
        Whether or not the input HEALPix data is noise and should be treated
        as such, by squaring and then rooting. Defaults to False.
    equinox: float
    ...
    # If the input data is noise, square it
    if is_noise:
        m **= 2
            ...
            for y in range(y_dim):
                ...
                for hpix_id in hpix_ids:
                    ...
                    if hpix_poly is None:
                        ...
                        sky_b = -1 * (theta_phi[0] * 360. / TAU - 90.)  # lat
                        sky_l = theta_phi[1] * 360. / TAU  # lon
    ...
    # If the input data was noise, root it and adjust it
    if is_noise:
        np.sqrt(data, data)
        hpx_pixel_area = hp.nside2pixarea(nside, degrees=True)
        wcs_pixel_area = abs(w.wcs.cdelt[0] * w.wcs.cdelt[1])
        noise_adjustment = sqrt(wcs_pixel_area / hpx_pixel_area)
        np.divide(data, noise_adjustment, data)

    # Write the output WCS data to file if the user asked for it
