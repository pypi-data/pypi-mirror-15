...
from rpcpdb.func_trace import Tracer
...
class UPdb_mixin(object):
    ...
    """
    # these are maps from function name to
    # (function, unix_socket_path/trace_path) pairs.
    _updb_debug_func = {}
    _updb_trace_func = {}
    ...
    def trace_func(self, f, tracepath, once=True,
                   match_criteria=None, ignore_count=0):
        """
        @param f: function name
        @param tracepath: pathname to write trace to
        @param once: one-shot debug - untrace after first hit
        @param match_criteria: dictionary of param name/value matches
        @param ignore_count: don't trigger for ignore_count matches
        @return: path to trace file
        """
        if f in self._updb_trace_func:
            return self._updb_trace_func[f][1]
        else:
            func = getattr(self, f)
            self._updb_trace_func[f] = (func, tracepath)
            ...
            def debug_check(*o, **k):
                if arg_match(o, k):
                    if debug_check._ignore_count <= 0:
                        ...
                        sys.settrace(Tracer(tracepath))
                        try:
                            return func(*o, **k)
                        finally:
                            sys.settrace(old_tracer)
                    debug_check._ignore_count -= 1
                # we're not tracing you, this time...
                return func(*o, **k)
            # use a function attribute to store the skip count;
            # decrement this on each matched call. Trigger at <= 0
            debug_check._ignore_count = ignore_count
    ...
    def untrace_func(self, f):
        """
        don't trace this function anymore.
        """
        if f in self._updb_trace_func:
            setattr(self, f, self._updb_trace_func.pop(f)[0])
