...
def _save_data(stabilizers,depth,loc,colors,branches,base,unique,c):
    from os import system, chdir
    """Saves the output in the right format for unit tests."""

    # save initial input
    system("mkdir check.in.{}".format(str(c)))
    chdir("check.in.{}".format(str(c)))
    for layer in range(len(stabilizers)):
        if layer <= depth:
            with open("_-G-layer-{}-perms".format(str(layer+1)),"w+") as lf:
                for perm in stabilizers[layer]:
                    p = perm[0]
                    lf.write(' '.join([str(i) for i in p]) + "\n")
        else:
            with open("_-G-layer-{}-.fpy.blank".format(str(layer+1)),"w+") as lf:
                lf.write('# <fortpy version="1" template="logical"></fortpy> \n F')

    with open("_-Gsize","w+") as gsf:
        for layer in range(len(stabilizers)):
            if layer <= depth:
                gsf.write(str(len(stabilizers[layer]))+ "    ")
            else:
                gsf.write("0    ")
    
    # save final output
    system("mkdir check.out.{}".format(str(c)))
    chdir("check.out.{}".format(str(c)))
...
def _hash(listi,color):
    """Generates a hash for each color in the ladeling returning the hash.
...
def brancher(concs,group,colors_w_arrows, dim, c, wanted, subset = []):
