...
#            'reachability' : self.check_reachability,
    ...
    def check_bounded_response_time(self, **kwargs):
                    ...
                    "filter": {
                        "bool": {
                            "must": [
                                {"term": {"src": source}},
                                {"term": {"dst": dest}},
                                { "regexp": {self.header: self.pattern}}
                            ]
                        }
                    }
                }
        ...
        for message in data["hits"]["hits"]:
            if _parse_duration(message['_source']["upstream_response_time"]) > max_latency:
                result = False
                # Request ID from service did not
                errormsg = "{} did not reply in time for request {}, {}".format(
                    dest, message['_source'][self.header], message['_source']["upstream_response_time"])
                if self.debug:
                    print errormsg
        return GremlinTestResult(result, errormsg)
    ...
    def check_bounded_retries(self, **kwargs):
        ...
        for bucket in data["aggregations"]["byid"]["buckets"]:
            ...
            req_seq.sort(key=lambda x: x['_source']["timestamp_in_ms"])
            for i in range(len(req_seq) - 1):
                observed = req_seq[i + 1]['_source']["timestamp_in_ms"] - req_seq[i]['_source']["timestamp_in_ms"]
