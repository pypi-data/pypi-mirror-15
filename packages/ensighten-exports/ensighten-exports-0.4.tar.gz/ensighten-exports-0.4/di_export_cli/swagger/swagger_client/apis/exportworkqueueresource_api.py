...
class ExportworkqueueresourceApi(object):
    ...
    def put_export_work_queue_using_put1(self, queue_id, entity, **kwargs):
        """
        putExportWorkQueue
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.put_export_work_queue_using_put1(queue_id, entity, callback=callback_function)
        ...
        all_params = ['queue_id', 'entity', 'x_auth_token']
        all_params.append('callback')
        ...
        for key, val in iteritems(params['kwargs']):
            if key not in all_params:
                raise TypeError(
                    "Got an unexpected keyword argument '%s'"
                    " to method put_export_work_queue_using_put1" % key
                )
            params[key] = val
        del params['kwargs']
        ...
        # verify the required parameter 'queue_id' is set
        if ('queue_id' not in params) or (params['queue_id'] is None):
            raise ValueError("Missing the required parameter `queue_id` when calling `put_export_work_queue_using_put1`")
        # verify the required parameter 'entity' is set
        if ('entity' not in params) or (params['entity'] is None):
            raise ValueError("Missing the required parameter `entity` when calling `put_export_work_queue_using_put1`")
