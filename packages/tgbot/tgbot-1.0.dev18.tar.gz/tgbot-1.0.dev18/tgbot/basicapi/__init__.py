...
def installlanguage(user_id,language:LanguageGetterAbstract=None):
    ...
    if not language.getlanguage(user_id):
        locale_path = os.path.dirname(pkg_resources.resource_filename(language.localedir,"__init__.py"))
        inter = gettext.translation(domain=language.standard,localedir=locale_path,languages=[language.standard])
    else:
        locale_path = os.path.dirname(pkg_resources.resource_filename(language.localedir,"__init__.py"))
        inter = gettext.translation(domain=language.getlanguage(user_id),localedir=locale_path,languages=[language.getlanguage(user_id)])
    inter.install()
