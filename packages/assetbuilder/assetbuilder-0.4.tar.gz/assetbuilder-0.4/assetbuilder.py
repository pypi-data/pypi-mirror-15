...
class AssetBuilder(object):
    ...
    def get_cachebuster(self, path):
        if self.autobuild and path not in self.cachebusters:
            self.ensure_up_to_date(path)
        return self.cachebusters.get(path, [0, ''])[1]
    ...
    def __call__(self, environ, start_response):
        ...
        p = self.served[path]
        if self.autobuild and not p.exists():
            self.ensure_up_to_date(path)
