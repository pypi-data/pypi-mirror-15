...
#         raise
    ...
    def initialize_grid_variables(self, gridUnitSize = None):
        ...
        self.box3DGrid = -1*np.ones(np.ceil(np.array(self.boxSize)/self.gridUnitSize), dtype=int)
...
#         raise
    ...
    def initialize_records_variables(self):
        ...
        numberOfRecords = np.sum(self.pdbsNumberOfRecordsInsertionDict.values())
...
#         raise        
    ...
    def parse_existing_pdbs(self):
        ...
        assert np.issubdtype(self.box3DGrid.dtype, int)
        
        Logger.info("Parsing existing pdbs")
        
        nExistingRecords = np.sum([len(pdb) for pdb in self.exisitingPdbs if isinstance(pdb, pdbParser)])
...
class AmorphousSystem(Construct):
    ...
    def construct(self):    
        ...
        totalAtomsNumber = np.sum(self.pdbsNumberOfRecordsInsertionDict.values())
        ...
        if self.priorities["density"]:
            ...
            Logger.info("insertion number is set to %r "%insertionDict.values())   
        else:
...
class Micelle(Construct):
    ...
    def construct(self):   
        ...
        for idx in range(len(spherePoints)):
            ...
            pdbMOL.models[pdbMOL.models.keys()[0]]["MODEL_NAME"] =  'MOL %s' %idx
