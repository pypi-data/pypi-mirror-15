...
class LdapUser(ldap_models.Model):
    ...
    def resync_devices(self):
        device_groups_by_group_dn = {dg.group_dn: dg for dg in LdapDeviceGroup.objects.all()}
        for group in LdapGroup.objects.filter(usernames__contains=self.username):
            if group.dn in device_groups_by_group_dn:
                device_groups_by_group_dn[group.dn].resync()
