...
class FaceSession(object):
    ...
    THRESHOLD_FACE_TO_SESSION=25.65
    SESSION_TO_PERSON_THRESHOLD=27.3
    SIGNATURE_LEN = 128.
    MAX_INACTIVE_SEC = 3600
    ...
    def __init__(self, db_manager):
        ...
        # While session is active, face_session_manager takes it into account
        # when looking for a session to match a new face.
        # A session becomes inactive after being idle for
        # FaceSessionManager.MAX_IDLE_SEC seconds. At that point, the session
        # still exists so that user can use it to create a new person, but
        # session will not be taken into account when trying to match a new face
        self.active = True

    def size(self):
        return len(self.db_face_ids)
    ...
    def close(self):
        """
        Close the session by either:
        1) committing all faces to best matching existing person  OR
        2) deleting all the faces from the DB since they don't match any person
        """
        print_and_log('closing session', DEBUG)

        # get a list of db faces objects using the ids list
        session_faces = [ self.db_manager.get_face_by_face_id(face_id)
                         for face_id in self.db_face_ids ]
        session_person, _ = self.find_best_person_for_session()
        if session_person is not None:
            for db_face in session_faces:
                db_face.person_id = session_person.id
        else:
            for db_face in session_faces:
    ...
    def create_new_person_and_close(self, name=None):
        db_person = DbPerson()
        if name:
            db_person.name = name
        ...
        for db_face in session_faces:
            if db_face is None:
                print_and_log('Oh-Oh! Got a NONE face! self.db_face_ids: {}'
                      ''.format(', '.join(str(id) for id in self.db_face_ids)))
                for id in self.db_face_ids:
                    print_and_log('ID: {}'.format(id))
                    face = self.db_manager.get_face_by_face_id(id)
                    print_and_log('FACE obj: {}'.format(face))
            db_face.person_id = db_person.id
    ...
    def find_best_person_for_session(self):
        """
        ...
        if (min_dist <= self.SESSION_TO_PERSON_THRESHOLD):
            return best_person, (100 - min_dist)
        else:
            return None, 0
