...
class BluetoothManager(threading.Thread):
    ...
    def disconnect_from_device(self):
        ...
        try:
            ...
            self.ble_adapter = BGAPIBackend()
            self.ble_adapter.start()
            print_and_log('Successfully disconnected from MyMe device')
        except Exception as ex:
    ...
    def write_command(self, command, args=0):
        """
        writes the given command to the device
        :param command: enum, one of constants.MyMeCommands
        :param command args (int)
        """
        self.req_queue.put(WriteCommandRequest(command, args))
    ...
    def _subscribe_to_service(self, service_uuid, callback=None, indication=True):
        """
        ...
        """
        if (self.myme_device.device_state not in [DeviceState.Connected]):
            print_and_log('ERROR: Cannot subscribe to services - device'
                              'is in an incorrect state ({})'
                              ''.format(self.myme_device.device_state), ERROR)
            return

        self.myme_device.handler.subscribe(service_uuid, callback, indication)
    ...
    def _handle_write_command_request(self, req):

        if (self.myme_device.device_state not in
                [DeviceState.Connected]):
             print_and_log('ERROR: cannot carry out request face image request: '
                               'Not connected to device.', ERROR)
        # pack the data, format is
        # 1 byte - command type
        # 4 bytes - command args (little endian)
        hex_packet = None
        try:
            hex_packet = bytearray(struct.pack("<BI", req.command, req.command_args))
            self.myme_device.handler.char_write(COMMAND_CHARACTERISTIC_UUID,
                                                    hex_packet)
        except Exception as ex:
            print_and_log('ERROR: Failed writng command to device '
                        '(hex packet: {}).\n '
                        'Exception Message: {}'.format(hex_packet, ex), ERROR)
    ...
    def _handle_face_signature_assembler_response(self, res):
        ...
        is_last_face_in_frame = (True if flag == 1 else False)
        try:
            self.sdk_face_signature_cb(face_id, timestamp, signature_int_array,
                                   is_last_face_in_frame)
        except Exception as ex:
            print_and_log('EXCEPTION calling sdk_face_signature_cb'
                          '\nException Message: {}'.format(ex))
    ...
    def _handle_face_image_assembler_response(self, res):
        ...
        curr_req_list = \
            self.pending_requests[RequestType.FaceImageRequest].pop(myme_face_id, None)
    ...
    def _handle_full_image_assembler_response(self, res):
        ...
        curr_req_list = \
            self.pending_requests[RequestType.FullImageRequest].pop(myme_image.frame_idx, None)
        for req in curr_req_list:
    ...
    def _handle_ocr_assembler_response(self, res):
        ...
        if flag == 1:
            self.sdk_ocr_line_cb(None, timestamp, True)
    ...
    def _handle_item(self, req):
        try:
            ...
            elif req.request_type == RequestType.WriteCommand:
                self._handle_write_command_request(req)
