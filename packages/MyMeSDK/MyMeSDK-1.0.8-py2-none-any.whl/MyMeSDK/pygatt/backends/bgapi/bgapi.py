...
class BGAPIBackend(BLEBackend):
    ...
    def reset(self):
        try:
            ...
            log.error('EXCEPTION when trying to reset device.\n'
    ...
    def disconnect_all(self):
        ...
        :return:
        """
        # print ('Disconnecting from all connection handlers')
    ...
    def connect(self, address, timeout=5,
        ...
        interval_min, interval_max -- ask for data from dongle at most
                    every interval_min ms and at least every interval_max ms.

        supervision_timeout -- This timeout determines the timeout from the
        last data exchange till a link is considered lost.
    ...
    def get_connection_status(self, connection_handle):
        self.send_command(CommandBuilder.connection_get_status(connection_handle))
        packet_type, reponse = self.expect(EventPacketType.connection_status)
        if not self._connection_status_flag(
                reponse['flags'],
                constants.connection_status_flag['connected']):
            return constants.BGAPI_device_state.disconnected
        else:
            return constants.BGAPI_device_state.connected
    ...
    def _scan_rsp_data(self, data):
        ...
        field_name = None
        field_value = []
        # Iterate over data bytes to _put in field
        dev_name = ""
        for b in data:
            ...
            if len(byte) > 0:
                ...
                if packet is not None:
                    ...
                    if packet_type == EventPacketType.attclient_attribute_value:
                        ...
                        # YY patch: if we just got disconnected and re-connected, we sometimes
                        # get an 'old' packet, of a connection that don't know about.
                        # in that case, just ignore it
                        if not self._connections.has_key(args['connection_handle']):
                           logging.warning("YY: Got packet for unknown connection {}, ignoring".
                                           format(args['connection_handle']))
                           continue
