...
from utils.sdk_utils import (db_face_to_myme_face, db_person_to_myme_person,
                             db_face_image_to_myme_face_image, ERROR)
from utils.appdirs import user_data_dir
from myme_device import MyMeData
from myme_objects import MyMeFaceImage, MyMePerson
...
def initialize(verbose=False):
    ...
    global db_manager
    global face_session_manager
    global face_session_manager_lock
    ...
    face_session_manager = FaceSessionMananger(db_manager=db_manager)
    face_session_manager_lock = threading.Lock()
...
def _handle_user_callback_queue_item(req):
    if req.type == UserCallbackReqType.PairedDeviceStateUpdatedCbReq:
        ...
        delegate.FaceDetected(req.face)
        ...
        if req.user_callback is not None:
            req.user_callback(req.myme_face_image)
...
def _sdk_ocr_line_cb(ocr_line, timestamp, is_frame_end=False):
    if (ocr_line is not None and
       'let my people go' in ocr_line.text.lower()):
        try:
            print 'Unpairing!!'
    ...
    user_callback_thread._put(OcrLineFoundCbReq(myme_ocr_line=ocr_line,
                            timestamp=timestamp, is_frame_end=is_frame_end))
...
def _sdk_face_signature_cb(detected_face_id, detected_time,
    ...
    :return:
    """
    with face_session_manager_lock:
        myme_face = face_session_manager.add_face_to_session(
                                                myme_face_id=detected_face_id,
                                                detected_time=detected_time,
                                                signature=signature_int_array)

    user_callback_thread._put(FaceDetectedCbReq(face=myme_face))
...
def _sdk_face_image_cb(face_db_id, detected_time,
    ...
    """
    myme_face_image = MyMeFaceImage(id=None,
                     detected_time=detected_time,
                     image=bytearray_to_image(image_bytearray),
                     face_id=face_db_id)
    user_callback_thread._put(
            FaceImageReceivedCbReq(myme_face_image, user_callback))
...
def activate_attentive_mode():
    """
    Activates 'attentive-mode' on the MyMe device
    """
    bt_manager.write_command(MyMeCommands.CommandEnterPowerMode)

# TODO - this is not implemented yet on device
def _deactivate_attentive_mode():
    bt_manager.write_command(MyMeCommands.CommnadExitPowerMode)


def poweroff_device():
    """
    Powers off the MyMe device
    """
    bt_manager.write_command(MyMeCommands.CommandShutdown)
...
def find_matching_person_for_face_id(face_id):
    """
    Searches for face_id in active/inactive sessions.
    If found, looks for best person match for the sessions signatures

    :param face_id: face_id of face (corresponding to a face temporarily stored
        in DB and part of a open (active or inactive) session
    :return: (myme_person, probability, face_represents_new_person)
             myme_person: (MyMePerson object) person matching face_id or None if not found
             probability: (float) Probability of match
             seen_enough_faces_to_create_new_person: (boolean) True if MyMe
             has seen enough faces of this person in order to effectively create
             a new person out of them. If False, the user is still allowed to
             create a new Person, however, since not enough faces have been
             seen, future recognition maybe ineffective.
    """
    _verify_initialization()
    with face_session_manager_lock:
        db_person, probability, seen_enough_faces_to_create_new_person = \
                 face_session_manager.find_matching_person_for_face_id(face_id)
    return (db_person_to_myme_person(db_person), probability,
            seen_enough_faces_to_create_new_person)


def create_person_with_face_id(face_id, name=None):
    """
    Searches for face_id in active/inactive sessions.
    If found, creates a new person from face's session and closes the session

    :param face_id:
    :param name: Optional string containing name to assign to the new person
    :return: MyMePerson object representing newly created DbPerson or None if not found
    """
    _verify_initialization()
    with face_session_manager_lock:
        db_person = face_session_manager.create_person_with_face_id(face_id, name)
...
#     """
#     Set automatic new person creation mode.
#
...
#     """
#     with face_session_manager_lock:
#         face_session_manager.auto_create_new_person = val
...
def set_profile_picture(my_me_person_id, profile_picture):
    """
    Sets the given profile picture to the given my_me_person_id.

    :param: my_me_person_id: id of the :class:`MyMeSDK.myme_objects.MyMePerson` object
    :param: profile_picture: the prfoile picture to set as binary jpeg content
    :return: None
    """
    _verify_initialization()
    db_person = db_manager.get_person_by_person_id(my_me_person_id)
    db_person.profile_picture = profile_picture
    db_manager.commit_db()
...
def clear_database():
    """
    Clear the entire database of the SDK. This deletes all people and faces stored!
...
def import_person_from_json(myme_person_json_path):
    """
    Imports a MyMePerson object along with all its data and images from
    provided json file

    :param myme_person_json_path: Path to JSON file containing serialized
           MyMePerson object
    :return: MyMePerson object imported from the json file
    """
    if not os.path.exists(myme_person_json_path):
        print_and_log('ERROR: Provided path: {} doesn\'t exist'
                      ''.format(myme_person_json_path), ERROR)
        return
    try:
        with open(myme_person_json_path, 'rb') as json_in:
            myme_person_dict = json.load(json_in)
            myme_person = MyMePerson.dict_to_person(myme_person_dict)
        db_person = DbPerson(name=myme_person.name,
                             profile_picture=myme_person.profile_picture)
        db_manager.add_and_commit(db_person)
        for face_img in myme_person.images:
            db_face_img = DbFaceImage(detected_time=face_img.detected_time,
                                      image=face_img.image)
            add_face_image_to_person(db_face_img, db_person.id)
        return myme_person
    except Exception as ex:
        print_and_log('Failed importing myme_person from json at: {}\n'
                      'Exception MSG: {}'.format(myme_person_json_path, ex), ERROR)
        return None


def export_person_to_json(myme_person, json_path):
    """
    Serializes myme_person into a json file at json_path. Afterwards, anyone can
    load this myme_person into their SDK using :meth:`MyMeSDK.api.import_person_from_json`

    :param myme_person: MyMePerson object to export to json file
    :param json_path: Path of json file to export to
    """
    try:
        myme_person_dict = myme_person.to_dict()
        with open(json_path, 'wb') as json_out:
            json.dump(myme_person_dict, json_out)

    except Exception as ex:
        print_and_log('Failed exporting myme_person to json at: {}'
                      '\nException MSG: {}'.format(json_path, ex), ERROR)
...
def create_person_with_image_urls(person_name, url_list, person_id=None):
    ...
    for url in url_list:
        try:
            ...
            if sig is not None:
                db_face_image_list.append(
                 DbFaceImage(image=image, detected_time=datetime.datetime.now()))

        except Exception as ex:
             print_and_log("Could not create signature from image file at: {}"
                           "\nException Message: {}".format(url, ex), ERROR)

    return _create_person_with_images_helper(person_name, db_face_image_list, person_id)
...
def create_person_with_images(person_name, file_paths_list, person_id=None):
    ...
    return _create_person_with_images_helper(person_name, db_face_image_list,
                                             person_id)


def _create_person_with_images_helper(person_name, db_face_image_list, person_id):

    # Ensure that we got at least one signature for the new person
    # (i.e one of the images was OK)
    if len(db_face_image_list) == 0:
    ...
    if person_id is None:
        ...
        if db_person is None:
            print_and_log('ERROR: person with id {} does not exist in the DB.\n'
                          'Creating a new person'.format(person_id), ERROR)
            person_id = None
            db_person = DbPerson(name=person_name)

    for face_image in db_face_image_list:
        db_person.images.append(face_image)

    # set the first image as profile pic
    if db_person.profile_picture is None:
        db_person.profile_picture = db_face_image_list[0].image

    db_person.images.extend(db_face_image_list)
...
def delete_face(db_face_id):
    """
    Delete the DbFace corresponding to face_id from the database.
    The reference to the face from the relevant person is also deleted.
    If not found does nothing.

    :param: db_face_id: the face id to delete
    """
    _verify_initialization()
    db_manager.delete_face_from_db(db_face_id)
...
def merge_people(person_id_list):
    ...
    for pers in db_personlist[1:]:
        db_personlist[0].faces.extend(pers.faces)
        db_personlist[0].images.extend(pers.images)
        if (pers.profile_picture is not None and
            db_personlist[0].profile_picture is None):
            db_personlist[0].profile_picture = pers.profile_picture
...
def add_face_image_to_person(myme_face_image, person_id):
    """
    Adds provided myme_face to person with person_id in the Person DB table

    :param person_id: ID in the Person DB of the person to whom myme_face belongs
    :param myme_face_image: MyMeFaceImage object representing the image to be added
    :return: True on success, False otherwise (probably if person_id is not found)
    """
    return db_manager.add_face_image_to_person(myme_face_image, person_id)

def get_person_id_for_face_id(face_id):
    """
    Gets person id of the person to whom the face with face_id belongs.

    :param face_id: Id of DbFace stored in the DB
    :return: id of DbPerson to whom the DbFace belongs, or None if not found
    """
    db_face = db_manager.get_face_by_face_id(face_id)
    if db_face is None:
        return None
    else:
        return db_face.person_id

def get_face_images_for_person(person_id):
    """
    Returns an array containing all face images stores for <person_id> in the DB
    :param person_id: ID of DbPerson in the Person DB for whom to fetch images
    :return: List of images for person_id or None if not found
    """
    return [db_face_image_to_myme_face_image(img)
            for img in db_manager.get_face_images_for_person(person_id)]


def remove_image_of_person(person_id, face_image_id):
    """
    Removes image with face_image_id belonging to person with person_id from SDK's DB
    :param person_id: ID of person for whom to remove image
    :param face_image_id: ID of image to remove
    """
    db_manager.remove_image_of_person(person_id, face_image_id)


def remove_all_images_of_person(person_id):
    """
    Removes all images stored for person_id in the SDK DB
    :param person_id: id of person in the SDK DB for whom to remove images
    """
    db_manager.remove_all_images_of_person(person_id)
...
def request_image_for_face_id(face_id, face_image_cb):
    ...
    >>> def face_image_cb(myme_face_image):
    ...    if myme_face_image.image is not None:
    ...         myme_face_image.image.show(title=
    ...             'id: {}, timestamp: {}'
    ...             ''.format(myme_face_image.id, myme_face_image.detected_time))
    ...
    db_face = db_manager.get_face_by_face_id(face_id)
    if db_face is None:
        print_and_log('ERROR: provided face_id doesn\'t exist, it has probably '
                      'expired. (Face was received too long ago to request image).',
                      ERROR)
        return
    bt_manager.request_face_image(
            db_face_id=db_face.id,
...
# On each registration, subscribe to these services
# TODO: FaceDetected - signatures and face_jpegs, OCR, BarCode, VisualTopics
# TODO: unregister functions for these 4 functionalities
# TODO: ensure thread-safety for the list which holds detection registration state
#       since it will be accessed (write) by user-thread and read by the BlueTooth thread
...
def _stop():
    ...
    global _is_initialized
    if not is_stopped:
        # stop the face_session_manager
        with face_session_manager_lock:
            face_session_manager.stop()
        # Seems like without the line below db_manager gets destroyed while still
        # inside the above function call
        print_and_log('{}'.format(db_manager))
