...
def quick_connect():
    ...
    print 'got sorted device list: {}'.format(sorted_device_list)
    if error:
        return False, error
...
def create_person_with_images(person_name, file_paths_list):
    """
    | This function creates a new person, and adds it to the database, using the
    | list of images provied in file_path_list.
    | This is useful to create a person with images retrieved not from the
    | MyMe device.
    ...
    :return: on success, returns the new :class:`MyMeSDK.myme_objects.MyMePerson` created. On failure, None is returned.
    """
    db_person = DbPerson(name=person_name)

    # for each file, create signature and image for it, and add to a new face object
    for file_path in file_paths_list:
        sig, image = CloudRequest.get_sig_from_image_file_path(file_path)
        if sig is not None:
            db_face = DbFace(signature=sig, image=image, detected_time=datetime.datetime.now())
            db_person.faces.append(db_face)

    # check if we got at least one signature for the new person (i.e one of the images was OK)
    if len(db_person.faces) == 0:
        print_and_log('ERROR: Could not create new person', ERROR)
        return None

    # set the first image as profile pic
    db_person.profile_pic = db_person.faces[0].image
