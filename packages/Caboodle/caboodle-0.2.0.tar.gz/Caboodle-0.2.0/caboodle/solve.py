...
class captcha(object):
	...
	def _solve(self, form):
		...
		if self.online or self.ocr:
			...
			img = Image.open(screen).crop((x, y, x + w, y + h))

			# Check for bar
			img = img.rotate(-90)

			try:
				iterator = iter(list(img.getdata())[:20])
				first = next(iterator)

				if all(first == rest for rest in iterator):
					temp = tempfile.NamedTemporaryFile(suffix = '.jpg')
					im = img.rotate(90)
					im = im.crop((0, im.height - 20, im.width, im.height))
			...
			# else
			img.crop((0, 18, img.width, img.height)).save(captcha, format = 'JPEG')
	...
	def solve(self):
		...
		try:
			...
			try:
				...
				self.browser.find_element_by_id('adcopy-link-refresh').click()
				return self.solve()
