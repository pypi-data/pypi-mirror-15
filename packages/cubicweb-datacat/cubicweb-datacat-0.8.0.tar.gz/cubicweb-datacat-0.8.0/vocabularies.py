...
EUROVOC_URI = u'http://publications.europa.eu/mdr/resource/thesaurus/eurovoc/skos/eurovoc_skos.zip'
    ...
    for subj_uri, pred_uri, obj_uri in eurovoc_graph.triples():
        ...
        if obj_uri == uri('eu:Domain'):
            ...
            for title in eurovoc_graph.objects(subj_uri, uri('skos:prefLabel')):
                if title.lang == u'fr':
                    new_graph.add(uriobject(subj_uri), uriobject(uri('dcterms:title')), title)
                    break
...
def eurovoc_domains(simplified_graph):
    ...
    for domain_uri in simplified_graph.uris_for_type(uri('skos:ConceptScheme')):
        for title in simplified_graph.objects(domain_uri, uri('skos:prefLabel')):
            if title.lang == u'fr':
                domains[domain_uri] = unicode(title)
                break
    return domains
