...
class Rsync:
    ...
    def transfer_to_remote(self, dest_dir, include=None, exclude=None, delete_excluded=False):
        if delete_excluded:
            delete_excluded = "--delete-excluded"
        else:
            delete_excluded = ""
        ...
        cmd = "rsync -avu %s %s %s %s %s" % (delete_excluded,
                                             include, exclude, dest_dir, self.remote_rsync_module)

        cr = ce.system(cmd)
        return cr.status

    def list_remote_files(self, include=None, exclude=None):
        if include:
            include = "--include=\"%s\"" % (include)
        else:
            include = ""
        ...
        cmd = "rsync -avu --list-only %s %s %s" % (
            include, exclude, self.remote_rsync_module)

        cr = ce.system(cmd)
        files = []
        if cr.status == 0 and cr.stdout_text:
